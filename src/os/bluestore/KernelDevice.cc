...
KernelDevice::KernelDevice(CephContext *cct, aio_callback_t cb, void *cbpriv,
    ...
    : BlockDevice(cct, cb, cbpriv), aio(false), dio(false),
      discard_callback(d_cb), discard_callback_priv(d_cbpriv), aio_stop(false),
      discard_started(false), discard_stop(false), aio_thread(this),
      discard_thread(this), injecting_crash(0) {
...
void KernelDevice::_aio_thread() {
  ...
  while (!aio_stop) {
    ...
    aio_t *aio[max];
    int r =
        io_queue->get_next_completed(cct->_conf->bdev_aio_poll_ms, aio, max);
    if (r < 0) {
...
void KernelDevice::aio_submit(IOContext *ioc) {
  ...
  int r, retries = 0;
  r = io_queue->submit_batch(ioc->running_aios.begin(), e, pending, priv,
                             &retries);
