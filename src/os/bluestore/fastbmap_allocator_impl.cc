...
inline interval_t _align2units(uint64_t offset, uint64_t len,
                               uint64_t min_length) {
  interval_t res;
  if (len >= min_length) {
  ...
  *tail = interval_t();
  ...
  do {
    ...
    if (bits & 1) {
      ...
      if (end_loop) {
        *tail = res_candidate;
        res_candidate = _align2units(res_candidate.offset, res_candidate.length,
                                     min_granules);
        if (res.length < res_candidate.length) {
          res = res_candidate;
        }
...
void AllocatorLevel01Loose::_analyze_partials(uint64_t pos_start,
                                              uint64_t pos_end, uint64_t length,
                                              uint64_t min_length, int mode,
                                              search_ctx_t *ctx) {
  ...
  for (auto pos = pos_start / d; pos < pos_end / d; ++pos) {
    ...
    for (auto c = 0; c < d; c++) {
      ...
      case L1_ENTRY_PARTIAL:
        ...
        longest = _get_longest_from_l0(l1_pos * l0_w, (l1_pos + 1) * l0_w,
                                       min_length, &prev_tail);
        ...
        if (longest.length >= min_length &&
          ...
          ctx->min_affordable_len =
              p2align<uint64_t>(longest.length, min_length);
...
interval_t AllocatorLevel01Loose::_allocate_l1_contiguous(uint64_t length,
  ...
  if (unlikely(length <= l0_granularity)) {
    ...
    search_ctx_t ctx;
    _analyze_partials(pos_start, pos_end, length, min_length, STOP_ON_EMPTY,
                      &ctx);
    ...
    if (ctx.min_affordable_len) {
      ...
      auto pos_end =
          (ctx.min_affordable_offs + ctx.min_affordable_len) / l0_granularity;
      _mark_alloc_l1_l0(pos_start, pos_end);
    ...
    if (ctx.free_count) {
      ...
      if (aligned_extent.length > 0) {
        aligned_extent.length =
            std::min(length, uint64_t(aligned_extent.length));
        ...
        auto pos_end =
            (aligned_extent.offset + aligned_extent.length) / l0_granularity;
...
bool AllocatorLevel01Loose::_allocate_l1(uint64_t length, uint64_t min_length,
                                         uint64_t max_length,
                                         uint64_t l1_pos_start,
                                         uint64_t l1_pos_end,
                                         uint64_t *allocated,
  ...
  if (min_length != l0_granularity) {
    ...
    for (auto idx = l1_pos_start / d1;
         idx < l1_pos_end / d1 && length > *allocated; ++idx) {
      slot_t &slot_val = l1[idx];
      if (slot_val == all_slot_clear) {
        continue;
      } else if (slot_val == all_slot_set) {
        uint64_t to_alloc = std::min(length - *allocated, l1_granularity * d1);
        *allocated += to_alloc;
      ...
      do {
        ...
        bool empty;
        empty = _allocate_l0(
            length, max_length, (idx * d1 + free_pos / L1_ENTRY_WIDTH) * l0_w,
            (idx * d1 + free_pos / L1_ENTRY_WIDTH + 1) * l0_w, allocated, res);
...
inline ssize_t AllocatorLevel01Loose::count_0s(slot_t slot_val,
                                               size_t start_pos) {
#ifdef __GNUC__
...
inline ssize_t AllocatorLevel01Loose::count_1s(slot_t slot_val,
                                               size_t start_pos) {
...
void AllocatorLevel01Loose::dump(
  ...
  for (size_t i = 0; i < l1.size(); i++) {
    for (size_t j = 0; j < L1_ENTRIES_PER_SLOT * L1_ENTRY_WIDTH;
         j += L1_ENTRY_WIDTH) {
      ...
      case L1_ENTRY_FREE:
        if (len == 0)
          off = ((bits_per_slot * i + j) / L1_ENTRY_WIDTH) * slots_per_slotset *
                bits_per_slot;
        len += bits_per_slotset;
        break;
      case L1_ENTRY_PARTIAL:
        size_t pos =
            ((bits_per_slot * i + j) / L1_ENTRY_WIDTH) * slots_per_slotset;
