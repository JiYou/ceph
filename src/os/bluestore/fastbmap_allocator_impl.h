...
struct interval_t {
  ...
  interval_t(const interval_t &ext) : offset(ext.offset), length(ext.length) {}
};
...
public:
  ...
  virtual void collect_stats(std::map<size_t, size_t> &bins_overall) = 0;
};
...
protected:
  ...
  uint64_t _level_granularity() const override { return l1_granularity; }
...
public:
  inline uint64_t get_min_alloc_size() const { return l0_granularity; }
};

template <class T> class AllocatorLevel02;
...
class AllocatorLevel01Loose : public AllocatorLevel01 {
  ...
  uint64_t _children_per_slot() const override { return L1_ENTRIES_PER_SLOT; }
  ...
  inline void _fragment_and_emplace(uint64_t max_length, uint64_t offset,
                                    uint64_t len, interval_vector_t *res) {
    auto it = res->rbegin();
    if (max_length) {
  ...
  bool _allocate_l0(uint64_t length, uint64_t max_length, uint64_t l0_pos0,
                    uint64_t l0_pos1, uint64_t *allocated,
    ...
    for (auto idx = l0_pos0 / d0; (idx < l0_pos1 / d0) && (length > *allocated);
      ...
      while (next_pos < bits_per_slot && (next_pos - free_pos) < need_entries) {
        ++l0_inner_iterations;
...
protected:
  ...
  void _init(uint64_t capacity, uint64_t _alloc_unit,
    ...
    auto aligned_capacity = p2roundup(
        (int64_t)capacity,
    ...
    if (mark_as_free) {
      ...
      auto l0_pos_no_use =
          p2roundup((int64_t)capacity, (int64_t)l0_granularity) /
          l0_granularity;
  ...
  void _analyze_partials(uint64_t pos_start, uint64_t pos_end, uint64_t length,
                         uint64_t min_length, int mode, search_ctx_t *ctx);
  ...
  void _mark_free_l0(int64_t l0_pos_start, int64_t l0_pos_end) {
    ...
    int64_t pos_e =
        std::min(l0_pos_end, p2roundup<int64_t>(l0_pos_start + 1, d0));
    while (pos < pos_e) {
      *val_s |= bits;
      bits <<= 1;
      pos++;
  ...
  interval_t _allocate_l1_contiguous(uint64_t length, uint64_t min_length,
                                     uint64_t max_length, uint64_t pos_start,
                                     uint64_t pos_end);

  bool _allocate_l1(uint64_t length, uint64_t min_length, uint64_t max_length,
                    uint64_t l1_pos_start, uint64_t l1_pos_end,
                    uint64_t *allocated, interval_vector_t *res);

  uint64_t _mark_alloc_l1(uint64_t offset, uint64_t length) {
    uint64_t l0_pos_start = offset / l0_granularity;
    uint64_t l0_pos_end =
        p2roundup(offset + length, l0_granularity) / l0_granularity;
  ...
  uint64_t _free_l1(uint64_t offs, uint64_t len) {
    ...
    uint64_t l0_pos_end =
        p2roundup(offs + len, l0_granularity) / l0_granularity;
...
public:
  ...
  void collect_stats(std::map<size_t, size_t> &bins_overall) override;
...
class AllocatorLevel01Compact : public AllocatorLevel01 {
  uint64_t _children_per_slot() const override { return 8; }
...
template <class L1> class AllocatorLevel02 : public AllocatorLevel {
public:
  ...
  uint64_t debug_get_allocated(uint64_t pos0 = 0, uint64_t pos1 = 0) {
    std::lock_guard l(lock);
    return l1.debug_get_allocated(
        pos0 * l1._children_per_slot() * bits_per_slot,
        pos1 * l1._children_per_slot() * bits_per_slot);
  }
  ...
  inline uint64_t get_min_alloc_size() const { return l1.get_min_alloc_size(); }
  void collect_stats(std::map<size_t, size_t> &bins_overall) override {
...
protected:
  ...
  uint64_t _children_per_slot() const override { return L1_ENTRIES_PER_SLOT; }
  uint64_t _level_granularity() const override { return l2_granularity; }
  ...
  void _init(uint64_t capacity, uint64_t _alloc_unit,
    ...
    auto aligned_capacity = p2roundup(
        (int64_t)capacity, (int64_t)l2_granularity * L1_ENTRIES_PER_SLOT);
    ...
    if (mark_as_free) {
      ...
      auto l2_pos_no_use =
          p2roundup((int64_t)capacity, (int64_t)l2_granularity) /
          l2_granularity;
  ...
  void _allocate_l2(uint64_t length, uint64_t min_length, uint64_t max_length,
                    uint64_t hint,
    ...
    for (auto i = 0; i < 2; ++i) {
      for (; length > *allocated && pos < pos_end; ++pos) {
        ...
        do {
          ceph_assert(length > *allocated);
          bool empty = l1._allocate_l1(
              length, min_length, max_length, (l2_pos + free_pos) * l1_w,
              (l2_pos + free_pos + 1) * l1_w, allocated, res);
          if (empty) {
  ...
  void _free_l2(const interval_set<uint64_t> &rr) {
    ...
    for (auto r : rr) {
      ...
      uint64_t l2_pos_end =
          p2roundup(int64_t(r.first + r.second), int64_t(l2_granularity)) /
          l2_granularity;
  ...
  template <typename T> void _free_l2(const T &rr) {
    uint64_t released = 0;
    std::lock_guard l(lock);
    for (auto r : rr) {
      ...
      uint64_t l2_pos_end =
          p2roundup(int64_t(r.offset + r.length), int64_t(l2_granularity)) /
          l2_granularity;
  ...
  void _mark_allocated(uint64_t o, uint64_t len) {
    ...
    uint64_t l2_pos_end =
        p2roundup(int64_t(o + len), int64_t(l2_granularity)) / l2_granularity;
