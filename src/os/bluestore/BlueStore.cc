 ...
 *
 */
template <typename S> static void append_escaped(const string &in, S *out) {
...
template <typename S> static string pretty_binary_string(const S &in) {
  char buf[10];
  string out;
...
template <typename T> static void _key_encode_shard(shard_id_t shard, T *key) {
...
static void get_coll_key_range(const coll_t &cid, int bits, string *temp_start,
                               string *temp_end, string *start, string *end) {
  temp_start->clear();
  temp_end->clear();
  start->clear();
  end->clear();
...
template <typename S> static int get_key_object(const S &key, ghobject_t *oid) {
  int r;
  const char *p = key.c_str();
...
static void get_object_key(CephContext *cct, const ghobject_t &oid, S *key) {
  ...
  size_t max_len = 1 + 8 + 4 + (oid.hobj.nspace.length() * 3 + 1) +
                   (oid.hobj.get_key().length() * 3 + 1) +
                   1 + // for '<', '=', or '>'
                   (oid.hobj.oid.name.length() * 3 + 1) + 8 + 8 + 1;
  key->reserve(max_len);
...
static void generate_extent_shard_key_and_apply(
    const S &onode_key, uint32_t offset, string *key,
...
int get_key_extent_shard(const string &key, string *onode_key,
                         uint32_t *offset) {
  ceph_assert(key.size() > sizeof(uint32_t) + 1);
  ceph_assert(*key.rbegin() == EXTENT_SHARD_KEY_SUFFIX);
...
void BlueStore::GarbageCollector::process_protrusive_extents(
    const BlueStore::ExtentMap &extent_map, uint64_t start_offset,
    uint64_t end_offset, uint64_t start_touch_offset, uint64_t end_touch_offset,
    uint64_t min_alloc_size) {
  ceph_assert(start_offset <= start_touch_offset &&
              end_offset >= end_touch_offset);
  ...
  dout(30) << __func__ << " (hex): [" << std::hex << lookup_start_offset << ", "
           << lookup_end_offset << ")" << std::dec << dendl;
  ...
  for (auto it = extent_map.seek_lextent(lookup_start_offset);
    ...
    dout(30) << __func__ << " " << *it << "alloc_units: " << alloc_unit_start
    ...
    if (it->logical_offset >= start_touch_offset &&
      ...
      BlobInfo &bi =
          affected_blobs.emplace(b, BlobInfo(b->get_referenced_bytes()))
              .first->second;
      ...
      dout(30) << __func__ << " affected_blob:" << *b << " unref 0x" << std::hex
               << it->length << " referenced = 0x" << bi.referenced_bytes
               << std::dec << dendl;
      ...
      if (blob_info_counted && used_alloc_unit == alloc_unit_start) {
        ...
        dout(30) << __func__ << " --expected_allocations:" << alloc_unit_start
                 << dendl;
      }
      used_alloc_unit = alloc_unit_end;
      blob_info_counted = nullptr;
  ...
  for (auto b_it = affected_blobs.begin(); b_it != affected_blobs.end();
       ++b_it) {
    Blob *b = b_it->first;
    ...
    if (bi.referenced_bytes == 0) {
      ...
      dout(30) << __func__ << " " << *(b_it->first)
               ...
               << " expected_allocations=" << bi.expected_allocations << dendl;
...
int64_t BlueStore::GarbageCollector::estimate(
    ...
    const BlueStore::old_extent_map_t &old_extents, uint64_t min_alloc_size) {
  ...
  for (auto it = old_extents.begin(); it != old_extents.end(); ++it) {
    ...
    if (b->get_blob().is_compressed()) {
      ...
      if (ref_bytes != 0) {
        dout(30) << __func__ << " affected_blob:" << *b << " unref 0x"
                 << std::hex << o << "~" << l << std::dec << dendl;
        affected_blobs.emplace(b, BlobInfo(ref_bytes));
      }
    }
  }
  dout(30) << __func__ << " gc range(hex): [" << std::hex << gc_start_offset
           << ", " << gc_end_offset << ")" << std::dec << dendl;
  ...
  if (gc_start_offset < start_offset || gc_end_offset > end_offset) {
    process_protrusive_extents(extent_map, gc_start_offset, gc_end_offset,
                               start_offset, end_offset, min_alloc_size);
  }
...
struct LruOnodeCacheShard : public BlueStore::OnodeCacheShard {
  typedef boost::intrusive::list<
      BlueStore::Onode,
      boost::intrusive::member_hook<BlueStore::Onode,
                                    boost::intrusive::list_member_hook<>,
                                    &BlueStore::Onode::lru_item>>
      list_t;
  typedef boost::intrusive::list<
      BlueStore::Onode,
      boost::intrusive::member_hook<BlueStore::Onode,
                                    boost::intrusive::list_member_hook<>,
                                    &BlueStore::Onode::pin_item>>
      pin_list_t;
...
// OnodeCacheShard
BlueStore::OnodeCacheShard *
BlueStore::OnodeCacheShard::create(CephContext *cct, string type,
                                   PerfCounters *logger) {
...
struct LruBufferCacheShard : public BlueStore::BufferCacheShard {
  typedef boost::intrusive::list<
      BlueStore::Buffer,
      boost::intrusive::member_hook<BlueStore::Buffer,
                                    boost::intrusive::list_member_hook<>,
                                    &BlueStore::Buffer::lru_item>>
      list_t;
  list_t lru;

  explicit LruBufferCacheShard(CephContext *cct)
  ...
  void add_stats(uint64_t *extents, uint64_t *blobs, uint64_t *buffers,
...
public:
  ...
  void _add(BlueStore::Buffer *b, int level, BlueStore::Buffer *near) override {
    dout(20) << __func__ << " level " << level << " near " << near << " on "
             << *b << " which has cache_private " << b->cache_private << dendl;
    if (near) {
  ...
  void _trim_to(uint64_t max) override {
    if (buffer_bytes > max) {
      ...
      if (evicted > 0) {
        dout(20) << __func__ << " evicted " << byte_u_t(evicted)
                 << " from hot list, done evicting hot buffers" << dendl;
      }
  ...
  void _audit(const char *s) override {
    ...
    if (hot_bytes != list_bytes[BUFFER_HOT]) {
      derr << __func__ << " hot_list_bytes " << list_bytes[BUFFER_HOT]
           << " != actual " << hot_bytes << dendl;
...
BlueStore::BufferCacheShard *
BlueStore::BufferCacheShard::create(CephContext *cct, string type,
                                    PerfCounters *logger) {
...
#undef dout_prefix
#define dout_prefix                                                            \
  *_dout << "bluestore.BufferSpace(" << this << " in " << cache << ") "
...
int BlueStore::BufferSpace::_discard(BufferCacheShard *cache, uint32_t offset,
                                     uint32_t length) {
  // note: we already hold cache->lock
    ...
    if (b->offset < offset) {
      ...
      if (b->end() > end) {
        ...
        if (b->data.length()) {
          ...
          _add_buffer(cache, new Buffer(this, b->state, b->seq, end, tail), 0,
                      b);
        }
        if (!b->is_writing()) {
...
void BlueStore::BufferSpace::read(BufferCacheShard *cache, uint32_t offset,
                                  uint32_t length,
...
void BlueStore::BufferSpace::split(BufferCacheShard *cache, size_t pos,
                                   BlueStore::BufferSpace &r) {
  ...
  while (true) {
    ...
    if (p->second->offset < pos) {
      ...
      if (p->second->data.length()) {
        ...
        r._add_buffer(cache,
                      new Buffer(&r, p->second->state, p->second->seq, 0, bl),
                      0, p->second.get());
      } else {
        r._add_buffer(
            cache, new Buffer(&r, p->second->state, p->second->seq, 0, right),
            0, p->second.get());
      }
    ...
    if (p->second->data.length()) {
      r._add_buffer(cache,
                    new Buffer(&r, p->second->state, p->second->seq,
...
#undef dout_prefix
#define dout_prefix                                                            \
  *_dout << "bluestore.OnodeSpace(" << this << " in " << cache << ") "
...
BlueStore::OnodeRef BlueStore::OnodeSpace::lookup(const ghobject_t &oid) {
...

  {
    ...
    if (p == onode_map.end()) {
      ...
      ldout(cache->cct, 30)
          << __func__ << " " << oid << " hit " << p->second << dendl;
...
void BlueStore::OnodeSpace::rename(
    OnodeRef &oldo, const ghobject_t &old_oid, const ghobject_t &new_oid,
...
void BlueStore::SharedBlob::put() {
  if (--nref == 0) {
    dout(20) << __func__ << " " << this << " removing self from set "
             << get_parent() << dendl;
  again:
    auto coll_snap = coll;
    if (coll_snap) {
...
void BlueStore::SharedBlob::finish_write(uint64_t seq) {
  while (true) {
    ...
    std::lock_guard l(cache->lock);
    if (coll->cache != cache) {
      dout(20) << __func__ << " raced with sb cache update, was " << cache
               << ", now " << coll->cache << ", retrying" << dendl;
      continue;
    }
...
void BlueStore::Blob::discard_unallocated(Collection *coll) {
  ...
  if (get_blob().is_compressed()) {
    ...
    for (auto e : get_blob().get_extents()) {
      if (!e.is_valid()) {
        dout(20) << __func__ << " 0x" << std::hex << pos << "~" << e.length
                 << std::dec << dendl;
...
void BlueStore::Blob::get_ref(Collection *coll, uint32_t offset,
                              uint32_t length) {
  ...
  used_in_blob.get(offset, length);
}

bool BlueStore::Blob::put_ref(Collection *coll, uint32_t offset,
                              uint32_t length, PExtentVector *r) {
  PExtentVector logical;

  dout(20) << __func__ << " 0x" << std::hex << offset << "~" << length
           << std::dec << " " << *this << dendl;

  bool empty = used_in_blob.put(offset, length, &logical);
  r->clear();
  // nothing to release
...
void BlueStore::Blob::split(Collection *coll, uint32_t blob_offset, Blob *r) {
  ...
  used_in_blob.split(blob_offset, &(r->used_in_blob));

  lb.split(blob_offset, rb);
  shared_blob->bc.split(shared_blob->get_cache(), blob_offset,
                        r->shared_blob->bc);
...
void BlueStore::Blob::decode(Collection *coll, bufferptr::const_iterator &p,
                             uint64_t struct_v, uint64_t *sbid,
...
// OldExtent
BlueStore::OldExtent *BlueStore::OldExtent::create(CollectionRef c, uint32_t lo,
...
void BlueStore::ExtentMap::dup(BlueStore *b, TransContext *txc,
                               CollectionRef &c, OnodeRef &oldo, OnodeRef &newo,
                               uint64_t &srcoff, uint64_t &length,
                               uint64_t &dstoff) {

  auto cct = onode->c->store->cct;
  bool inject_21040 = cct->_conf->bluestore_debug_inject_bug21040;
  ...
  for (auto ep = oldo->extent_map.seek_lextent(srcoff);
       ep != oldo->extent_map.extent_map.end(); ++ep) {
    auto &e = *ep;
    ...
    if (e.blob->last_encoded_id >= 0) {
      ...
      if (!blob.is_shared()) {
        ...
        } else if (inject_21040 && dirty_range_begin == 0 &&
                   dirty_range_end == 0) {
          dirty_range_begin = e.logical_offset;
        }
...
void BlueStore::ExtentMap::update(KeyValueDB::Transaction t, bool force) {
  ...
  if (onode->onode.extent_map_shards.empty()) {
    ...
    for (auto &it : encoded_shards) {
      ...
      generate_extent_shard_key_and_apply(
          onode->key, it.shard->shard_info->offset, &key,
...
void BlueStore::ExtentMap::reshard(KeyValueDB *db, KeyValueDB::Transaction t) {
  auto cct = onode->c->store->cct; // used by dout

  dout(10) << __func__ << " 0x[" << std::hex << needs_reshard_begin << ","
           << needs_reshard_end << ")" << std::dec << " of "
           << onode->onode.extent_map_shards.size() << " shards on "
           << onode->oid << dendl;
  ...
  fault_range(db, needs_reshard_begin,
              (needs_reshard_end - needs_reshard_begin));
  ...
  for (unsigned i = si_begin; i < si_end; ++i) {
    generate_extent_shard_key_and_apply(
        ...
        [&](const string &final_key) { t->rmkey(PREFIX_OBJ, final_key); });
  }
  ...
  unsigned slop =
      target * cct->_conf->bluestore_extent_map_shard_target_size_slop;
  ...
  if (new_shard_info.empty() && (si_begin > 0 || si_end < shards.size())) {
  ...
  if (sv.empty()) {
    ...
    sv.insert(sv.begin() + si_begin, new_shard_info.begin(),
              new_shard_info.end());
  ...
  if (sv.empty()) {
    ...
    if (spanning_scan_end > needs_reshard_end) {
      fault_range(db, needs_reshard_end, spanning_scan_end - needs_reshard_end);
    }
    ...
    decltype(onode->c->onode_map.cache->dumped_onodes)::value_type *oid_slot =
        nullptr;
    decltype(
        onode->c->onode_map.cache->dumped_onodes)::value_type *oldest_slot =
        nullptr;
      ...
      if (e->blob_escapes_range(shard_start, shard_end - shard_start)) {
        if (!e->blob->is_spanning()) {
          ...
          if (must_span) {
            ...
            if (!was_too_many_blobs_check && too_many_blobs_threshold &&
              ...
              for (size_t i = 0; i < dumped_onodes.size(); ++i) {
                ...
                if (!oldest_slot || (oldest_slot && dumped_onodes[i].second <
                                                        oldest_slot->second)) {
    ...
    bool do_dump = (!oid_slot && was_too_many_blobs_check) ||
                   (oid_slot && (mono_clock::now() - oid_slot->second >=
                                 make_timespan(5 * 60)));
    if (do_dump) {
      dout(0) << __func__ << " spanning blob count exceeds threshold, "
              << spanning_blob_map.size() << " spanning blobs" << dendl;
      _dump_onode<0>(cct, *onode);
      if (oid_slot) {
...
bool BlueStore::ExtentMap::encode_some(uint32_t offset, uint32_t length,
  ...
  for (auto p = start; p != extent_map.end() && p->logical_offset < end;
       ++p, ++n) {
    ...
    if (!must_reshard) {
      ...
      p->blob->bound_encode(bound, struct_v, p->blob->shared_blob->get_sbid(),
                            false);
    }
  }
  if (must_reshard) {
  ...
  */
  ...
  while (!p.end()) {
    ...
    if (blobid & BLOBID_FLAG_SPANNING) {
      ...
      le->blob->get_ref(onode->c, le->blob_offset, le->length);
    }
    pos += prev_len;
    ++n;
...
void BlueStore::ExtentMap::bound_encode_spanning_blobs(size_t &p) {
  ...
  for (const auto &i : spanning_blob_map) {
    i.second->bound_encode(p, struct_v, i.second->shared_blob->get_sbid(),
                           true);
  }
}
...
void BlueStore::ExtentMap::decode_spanning_blobs(bufferptr::const_iterator &p) {
  __u8 struct_v;
  denc(struct_v, p);
  // Version 2 differs from v1 in blob's ref_map
  // serialization only. Hence there is no specific
...
void BlueStore::ExtentMap::fault_range(KeyValueDB *db, uint32_t offset,
                                       uint32_t length) {
  dout(30) << __func__ << " 0x" << std::hex << offset << "~" << length
           << std::dec << dendl;
  ...
  while (start <= last) {
    ...
    if (!p->loaded) {
      ...
      dout(20) << __func__ << " open shard 0x" << std::hex
               << p->shard_info->offset << " for range 0x" << offset << "~"
               << length << std::dec << " (" << v.length() << " bytes)"
               << dendl;
...
void BlueStore::ExtentMap::dirty_range(uint32_t offset, uint32_t length) {
  dout(30) << __func__ << " 0x" << std::hex << offset << "~" << length
           << std::dec << dendl;
  if (shards.empty()) {
  ...
  while (start <= last) {
    ...
    auto p = &shards[start];
    if (!p->loaded) {
      derr << __func__ << "on write 0x" << std::hex << offset << "~" << length
           << " shard 0x" << p->shard_info->offset << std::dec
...
BlueStore::extent_map_t::iterator BlueStore::ExtentMap::find(uint64_t offset) {
  Extent dummy(offset);
  return extent_map.find(dummy);
}

BlueStore::extent_map_t::iterator
BlueStore::ExtentMap::seek_lextent(uint64_t offset) {
  Extent dummy(offset);
...
BlueStore::extent_map_t::const_iterator
BlueStore::ExtentMap::seek_lextent(uint64_t offset) const {
  Extent dummy(offset);
  auto fp = extent_map.lower_bound(dummy);
  if (fp != extent_map.begin()) {
    --fp;
...
int BlueStore::ExtentMap::compress_extent_map(uint64_t offset,
                                              uint64_t length) {
  if (extent_map.empty())
    return 0;
  int removed = 0;
  ...
  for (++n; n != extent_map.end(); p = n++) {
    ...
    while (n != extent_map.end() && p->logical_end() == n->logical_offset &&
           p->blob == n->blob && p->blob_offset + p->length == n->blob_offset &&
           n->logical_offset < shard_end) {
      dout(20) << __func__ << " 0x" << std::hex << offset << "~" << length
               << " next shard 0x" << shard_end << std::dec << " merging " << *p
               << " and " << *n << dendl;
      p->length += n->length;
      rm(n++);
      ++removed;
...
void BlueStore::ExtentMap::punch_hole(CollectionRef &c, uint64_t offset,
                                      uint64_t length,
  ...
  while (p != extent_map.end()) {
    ...
    if (p->logical_offset < offset) {
      if (p->logical_end() > end) {
        ...
        add(end, p->blob_offset + front + length, p->length - front - length,
            p->blob);
        p->length = front;
        break;
      } else {
...
BlueStore::Extent *
BlueStore::ExtentMap::set_lextent(CollectionRef &c, uint64_t logical_offset,
                                  uint64_t blob_offset, uint64_t length,
                                  BlobRef b, old_extent_map_t *old_extents) {
...
BlueStore::BlobRef BlueStore::ExtentMap::split_blob(BlobRef lb,
  ...
  for (auto ep = seek_lextent(pos);
       ep != extent_map.end() && ep->logical_offset < end_pos; ++ep) {
    if (ep->blob != lb) {
      continue;
    }
...
#undef dout_prefix
#define dout_prefix                                                            \
  *_dout << "bluestore.onode(" << this << ")." << __func__ << " "

BlueStore::Onode *BlueStore::Onode::decode(CollectionRef c,
...
bool BlueStore::WriteContext::has_conflict(BlobRef b, uint64_t loffs,
                                           uint64_t loffs_end,
                                           uint64_t min_alloc_size) {
...
void BlueStore::DeferredBatch::prepare_write(CephContext *cct, uint64_t seq,
  ...
  dout(20) << __func__ << " seq " << seq << " 0x" << std::hex << offset << "~"
           << length << " crc " << i.first->second.bl.crc32c(-1) << std::dec
           << dendl;
...
void BlueStore::DeferredBatch::_discard(CephContext *cct, uint64_t offset,
                                        uint64_t length) {
  ...
  if (p != iomap.begin()) {
    ...
    if (end > offset) {
      ...
      dout(20) << __func__ << "  keep head " << p->second.seq << " 0x"
               << std::hex << p->first << "~" << p->second.bl.length()
      ...
      if (end > offset + length) {
        ...
        dout(20) << __func__ << "  keep tail " << p->second.seq << " 0x"
                 << std::hex << p->first << "~" << p->second.bl.length()
  ...
  while (p != iomap.end()) {
    ...
    if (end > offset + length) {
      ...
      dout(20) << __func__ << "  truncate front " << p->second.seq << " 0x"
               << std::hex << p->first << "~" << p->second.bl.length()
               << " drop_front 0x" << drop_front << " keep_tail 0x" << keep_tail
               << " to 0x" << (offset + length) << "~" << keep_tail << std::dec
               << dendl;
      ...
      dout(20) << __func__ << "  drop " << p->second.seq << " 0x" << std::hex
               << p->first << "~" << p->second.bl.length() << std::dec << dendl;
...
#undef dout_prefix
#define dout_prefix                                                            \
  *_dout << "bluestore(" << store->path << ").collection(" << cid << " "       \
         << this << ") "

BlueStore::Collection::Collection(BlueStore *store_, OnodeCacheShard *oc,
                                  BufferCacheShard *bc, coll_t cid)
    : CollectionImpl(store_->cct, cid), store(store_), cache(bc), exists(true),
      onode_map(oc), commit_queue(nullptr) {}

bool BlueStore::Collection::flush_commit(Context *c) {
  return osr->flush_commit(c);
}

void BlueStore::Collection::flush() { osr->flush(); }

void BlueStore::Collection::flush_all_but_last() { osr->flush_all_but_last(); }
...
void BlueStore::Collection::open_shared_blob(uint64_t sbid, BlobRef b) {
  ...
  if (b->shared_blob) {
    ...
    ldout(store->cct, 10) << __func__ << " sbid 0x" << std::hex << sbid
                          << std::dec << " opened " << *b->shared_blob << dendl;
  }
}
...
BlueStore::OnodeRef BlueStore::Collection::get_onode(const ghobject_t &oid,
                                                     bool create,
                                                     bool is_createop) {
...
void BlueStore::Collection::split_cache(Collection *dest) {
  ...
  while (p != onode_map.onode_map.end()) {
    ...
    if (!p->second->oid.match(destbits, destpg.pgid.ps())) {
      ...
      ldout(store->cct, 20)
          << __func__ << " not moving " << o << " " << o->oid << dendl;
      ++p;
    } else {
      ldout(store->cct, 20)
          << __func__ << " moving " << o << " " << o->oid << dendl;
      ...
      for (auto sb : sbvec) {
        if (sb->coll == dest) {
          ldout(store->cct, 20)
              << __func__ << "  already moved " << *sb << dendl;
          continue;
        }
        ldout(store->cct, 20) << __func__ << "  moving " << *sb << dendl;
        if (sb->get_sbid()) {
          ldout(store->cct, 20)
              << __func__ << "   moving registration " << *sb << dendl;
          shared_blob_set.remove(sb);
...
void *BlueStore::MempoolThread::entry() {
  ...
  if (store->cache_autotune && binned_kv_cache != nullptr) {
    pcm = std::make_shared<PriorityCache::Manager>(store->cct, min, max, target,
                                                   true);
  ...
  while (!stop) {
    ...
    auto wait =
        ceph::make_timespan(store->cct->_conf->bluestore_cache_trim_interval);
...
void BlueStore::MempoolThread::_resize_shards(bool interval_stats) {
  ...
  int64_t kv_alloc = static_cast<int64_t>(store->cache_kv_ratio * cache_size);
  int64_t meta_alloc =
  ...
  if (interval_stats) {
    dout(5) << __func__ << " cache_size: " << cache_size
            << " kv_alloc: " << kv_alloc << " kv_used: " << kv_used
            << " meta_alloc: " << meta_alloc << " meta_used: " << meta_used
            << " data_alloc: " << data_alloc << " data_used: " << data_used
            << dendl;
  } else {
...
BlueStore::OmapIteratorImpl::OmapIteratorImpl(CollectionRef c, OnodeRef o,
...
string BlueStore::OmapIteratorImpl::_stringify() const {
  stringstream s;
  s << " omap_iterator(cid = " << c->cid << ", oid = " << o->oid << ")";
  return s.str();
}
...
int BlueStore::OmapIteratorImpl::seek_to_first() {
  ...
  c->store->log_latency(__func__, l_bluestore_omap_seek_to_first_lat,
...
int BlueStore::OmapIteratorImpl::upper_bound(const string &after) {
  ...
  c->store->log_latency_fn(
      __func__, l_bluestore_omap_upper_bound_lat, mono_clock::now() - start1,
      c->store->cct->_conf->bluestore_log_omap_iterator_age,
      [&](const ceph::timespan &lat) {
        return ", after = " + after + _stringify();
      });
  return 0;
}
...
int BlueStore::OmapIteratorImpl::lower_bound(const string &to) {
  ...
  c->store->log_latency_fn(
      __func__, l_bluestore_omap_lower_bound_lat, mono_clock::now() - start1,
      c->store->cct->_conf->bluestore_log_omap_iterator_age,
      [&](const ceph::timespan &lat) { return ", to = " + to + _stringify(); });
  return 0;
}
...
bool BlueStore::OmapIteratorImpl::valid() {
  std::shared_lock l(c->lock);
  bool r =
      o->onode.has_omap() && it && it->valid() && it->raw_key().second < tail;
  if (it && it->valid()) {
    ldout(c->store->cct, 20)
        << __func__ << " is at " << pretty_binary_string(it->raw_key().second)
        << dendl;
  }
  return r;
}
...
int BlueStore::OmapIteratorImpl::next() {
  ...
  c->store->log_latency(__func__, l_bluestore_omap_next_lat,
                        mono_clock::now() - start1,
                        c->store->cct->_conf->bluestore_log_omap_iterator_age);
...
BlueStore::BlueStore(CephContext *cct, const string &path,
    ...
    : ObjectStore(cct, path), throttle(cct),
      finisher(cct, "commit_finisher", "cfin"), kv_sync_thread(this),
      kv_finalize_thread(this), min_alloc_size(_min_alloc_size),
      min_alloc_size_order(ctz(_min_alloc_size)), mempool_thread(this) {
  _init_logger();
...
const char **BlueStore::get_tracked_conf_keys() const {
  static const char *KEYS[] = {"bluestore_csum_type",
...
void BlueStore::handle_conf_change(const ConfigProxy &conf,
  ...
  if (changed.count("osd_memory_target") || changed.count("osd_memory_base") ||
...
void BlueStore::_set_compression() {
  auto m =
      Compressor::get_comp_mode_type(cct->_conf->bluestore_compression_mode);
  if (m) {
    ...
    derr << __func__ << " unrecognized value '"
         ...
         << "' for bluestore_compression_mode, reverting to 'none'" << dendl;
  ...
  if (!alg_name.empty()) {
    ...
    if (!compressor) {
      derr << __func__ << " unable to initialize " << alg_name.c_str()
           << " compressor" << dendl;
  ...
  dout(10) << __func__ << " mode " << Compressor::get_comp_mode_name(comp_mode)
           ...
           << " min_blob " << comp_min_blob_size << " max_blob "
           << comp_max_blob_size << dendl;
}
...
void BlueStore::_set_csum() {
  ...
  dout(10) << __func__ << " csum_type "
           << Checksummer::get_csum_type_string(csum_type) << dendl;
}
...
void BlueStore::_update_osd_memory_options() {
  ...
  osd_memory_cache_min =
      cct->_conf.get_val<Option::size_t>("osd_memory_cache_min");
  config_changed++;
  dout(10) << __func__ << " osd_memory_target " << osd_memory_target
           << " osd_memory_base " << osd_memory_base
           << " osd_memory_expected_fragmentation "
           << osd_memory_expected_fragmentation << " osd_memory_cache_min "
           << osd_memory_cache_min << dendl;
}
...
int BlueStore::_set_cache_sizes() {
  ...
  dout(1) << __func__ << " cache_size " << cache_size << " meta "
          << cache_meta_ratio << " kv " << cache_kv_ratio << " data "
          << cache_data_ratio << dendl;
  return 0;
}
...
void BlueStore::_init_logger() {
  PerfCountersBuilder b(cct, "bluestore", l_bluestore_first, l_bluestore_last);
  ...
  b.add_time_avg(l_bluestore_kv_sync_lat, "kv_sync_lat",
                 "Average kv_sync thread latency", "ks_l",
                 PerfCountersBuilder::PRIO_INTERESTING);
  b.add_time_avg(l_bluestore_kv_final_lat, "kv_final_lat",
                 "Average kv_finalize thread latency", "kf_l",
                 PerfCountersBuilder::PRIO_INTERESTING);
  ...
  b.add_time_avg(l_bluestore_state_deferred_aio_wait_lat,
  ...
  b.add_time_avg(l_bluestore_throttle_lat, "throttle_lat",
                 "Average submit throttle latency", "th_l",
                 PerfCountersBuilder::PRIO_CRITICAL);
  b.add_time_avg(l_bluestore_submit_lat, "submit_lat", "Average submit latency",
                 "s_l", PerfCountersBuilder::PRIO_CRITICAL);
  b.add_time_avg(l_bluestore_commit_lat, "commit_lat", "Average commit latency",
  ...
  b.add_time_avg(l_bluestore_csum_lat, "csum_lat", "Average checksum latency");
  b.add_u64_counter(l_bluestore_compress_success_count,
                    "compress_success_count",
                    "Sum for beneficial compress ops");
  b.add_u64_counter(
      l_bluestore_compress_rejected_count, "compress_rejected_count",
      "Sum for compress ops rejected due to low net gain of space");
  b.add_u64_counter(l_bluestore_write_pad_bytes, "write_pad_bytes",
                    "Sum for write-op padded bytes", NULL, 0,
                    unit_t(UNIT_BYTES));
  ...
  b.add_u64_counter(l_bluestore_deferred_write_bytes, "deferred_write_bytes",
                    "Sum for deferred write bytes", "def", 0,
                    unit_t(UNIT_BYTES));
  b.add_u64_counter(l_bluestore_write_penalty_read_ops,
  ...
  b.add_u64(l_bluestore_compressed, "bluestore_compressed",
            "Sum for stored compressed bytes", "c",
  ...
  b.add_u64(l_bluestore_compressed_allocated, "bluestore_compressed_allocated",
            "Sum for bytes allocated for compressed data", "c_a",
            PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_u64(l_bluestore_compressed_original, "bluestore_compressed_original",
            "Sum for original bytes that were compressed", "c_o",
            PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  ...
  b.add_u64(l_bluestore_blobs, "bluestore_blobs", "Number of blobs in cache");
  ...
  b.add_u64_counter(l_bluestore_buffer_hit_bytes, "bluestore_buffer_hit_bytes",
                    "Sum for bytes of read hit in the cache", NULL, 0,
                    unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluestore_buffer_miss_bytes, "bluestore_buffer_miss_bytes",
  ...
  b.add_u64_counter(l_bluestore_write_big_bytes, "bluestore_write_big_bytes",
                    "Large aligned writes into fresh blobs (bytes)", NULL, 0,
                    unit_t(UNIT_BYTES));
  ...
  b.add_u64_counter(l_bluestore_write_small_bytes,
  ...
  b.add_u64_counter(
      l_bluestore_extent_compress, "bluestore_extent_compress",
  ...
  b.add_u64_counter(l_bluestore_reads_with_retries,
                    "bluestore_reads_with_retries",
                    "Read operations that required at least one retry due to "
...
int BlueStore::_reload_logger() {
  ...
  if (r >= 0) {
    ...
    logger->set(l_bluestore_compressed_allocated,
                store_statfs.data_compressed_allocated);
    logger->set(l_bluestore_compressed_original,
                store_statfs.data_compressed_original);
  }
  return r;
}
...
int BlueStore::_write_bdev_label(CephContext *cct, string path,
                                 bluestore_bdev_label_t label) {
  ...
  r = ::fsync(fd);
  if (r < 0) {
    derr << __func__ << " failed to fsync " << path << ": " << cpp_strerror(r)
         << dendl;
  }
out:
...
int BlueStore::_check_or_set_bdev_label(string path, uint64_t size, string desc,
                                        bool create) {
  bluestore_bdev_label_t label;
  if (create) {
    label.osd_uuid = fsid;
...
void BlueStore::_set_alloc_sizes(void) {
  ...
  dout(10) << __func__ << " min_alloc_size 0x" << std::hex << min_alloc_size
           ...
           << " prefer_deferred_size 0x" << prefer_deferred_size << std::dec
           << " deferred_batch_ops " << deferred_batch_ops << dendl;
}
...
int BlueStore::_open_bdev(bool create) {
  ...
  bdev = BlockDevice::create(cct, p, aio_cb, static_cast<void *>(this),
                             discard_cb, static_cast<void *>(this));
  int r = bdev->open(p);
  if (r < 0)
    goto fail;
...
void BlueStore::_validate_bdev() {
  ...
  if (dev_size < _get_ondisk_reserved() + cct->_conf->bluestore_bluefs_min) {
    dout(1) << __func__ << " main device size " << byte_u_t(dev_size)
            << " is too small, disable bluestore_bluefs_min for now" << dendl;
...
int BlueStore::_open_fm(KeyValueDB::Transaction t) {
  ...
  if (t) {
    ...
    if (cct->_conf->bluestore_debug_prefill > 0) {
      ...
      uint64_t max_b =
          cct->_conf->bluestore_debug_prefragment_max / min_alloc_size;
  ...
  if (fm->get_size() >= bdev->get_size() + min_alloc_size) {
    ...
    ss << "slow device size mismatch detected, "
       << " fm size(" << fm->get_size() << ") > slow device size("
       << bdev->get_size()
...
int BlueStore::_open_alloc() {
  ...
  if (bluefs) {
    ...
    if (r < 0) {
      lderr(cct) << __func__
                 << " failed to retrieve bluefs_extents: " << cpp_strerror(r)
                 << dendl;

      return r;
    }
    dout(10) << __func__ << " bluefs extents 0x" << std::hex << bluefs_extents
             << std::dec << dendl;
  }

  alloc = Allocator::create(cct, cct->_conf->bluestore_allocator,
                            bdev->get_size(), min_alloc_size, "block");
  if (!alloc) {
    lderr(cct) << __func__ << " Allocator::unknown alloc type "
               << cct->_conf->bluestore_allocator << dendl;
    return -EINVAL;
  }
  ...
  fm->enumerate_reset();
  dout(1) << __func__ << " loaded " << byte_u_t(bytes) << " in " << num
          << " extents" << dendl;
...
int BlueStore::_lock_fsid() {
  ...
  if (r < 0) {
    ...
    derr << __func__ << " failed to lock " << path << "/fsid"
         << " (is another ceph-osd still running?)" << cpp_strerror(err)
         << dendl;
    return -err;
  }
  return 0;
...
int BlueStore::_minimal_open_bluefs(bool create) {
  ...
  if (::stat(bfn.c_str(), &st) == 0) {
    ...
    if (r < 0) {
      derr << __func__ << " add block device(" << bfn
           << ") returned: " << cpp_strerror(r) << dendl;
      goto free_bluefs;
    }

    if (bluefs->bdev_support_label(BlueFS::BDEV_DB)) {
      r = _check_or_set_bdev_label(
          bfn, bluefs->get_block_device_size(BlueFS::BDEV_DB), "bluefs db",
          create);
      if (r < 0) {
    ...
    if (create) {
      bluefs->add_block_extent(BlueFS::BDEV_DB, SUPER_RESERVED,
    ...
    if (::lstat(bfn.c_str(), &st) == -1) {
      ...
      derr << __func__ << " " << bfn
           << " symlink exists but target unusable: " << cpp_strerror(r)
           << dendl;
      goto free_bluefs;
    }
  }
  ...
  if (::stat(bfn.c_str(), &st) == 0) {
    ...
    if (bluefs->bdev_support_label(BlueFS::BDEV_WAL)) {
      r = _check_or_set_bdev_label(
          bfn, bluefs->get_block_device_size(BlueFS::BDEV_WAL), "bluefs wal",
          create);
      if (r < 0) {
        derr << __func__ << " check block device(" << bfn
             << ") label returned: " << cpp_strerror(r) << dendl;
...
int BlueStore::_open_bluefs(bool create) {
  ...
  if (bluefs_layout.shared_bdev == BlueFS::BDEV_SLOW) {
    ...
    int r = RocksDBStore::ParseOptionsFromStringStatic(cct, options, rocks_opts,
                                                       nullptr);
    if (r < 0) {
      return r;
    }
    ...
    vselector = new RocksDBBlueFSVolumeSelector(
        ...
        rocks_opts.max_bytes_for_level_multiplier, reserved_factor,
...
void BlueStore::_sync_bluefs_and_fm() {
  if (cct->_conf->bluestore_bluefs_db_compatibility) {
    ...
    dout(20) << __func__ << " bluefs_extents at KV is now 0x" << std::hex
...
int BlueStore::_open_db(bool create, bool to_repair_db, bool read_only) {
  ...
  if (do_bluefs) {
    ...
    if (cct->_conf->bluestore_bluefs_env_mirror) {
      ...
      if (create) {
        string cmd =
            "rm -rf " + path + "/db " + path + "/db.slow " + path + "/db.wal";
  ...
  db = KeyValueDB::create(cct, kv_backend, fn, kv_options,
  ...
  dout(1) << __func__ << " opened " << kv_backend << " path " << fn
          << " options " << options << dendl;
  return 0;
}
...
void BlueStore::_dump_alloc_on_failure() {
  auto dump_interval = cct->_conf->bluestore_bluefs_alloc_failure_dump_interval;
  if (dump_interval > 0 &&
...
int BlueStore::allocate_bluefs_freespace(uint64_t min_size, uint64_t size,
  ...
  if (size) {
    ...
    do {
      ...
      dout(10) << __func__ << " gifting " << gift << " (" << byte_u_t(gift)
               << ")" << dendl;
      ...
      if (alloc_len < 0 ||
        ...
        derr << __func__ << " failed to allocate on 0x" << std::hex << gift
             << " min_size 0x" << min_size << " > allocated total 0x"
             << allocated << " bluefs_shared_alloc_size 0x" << alloc_size
             << " allocated 0x" << (alloc_len < 0 ? 0 : alloc_len)
             << " available 0x " << alloc->get_free() << std::dec << dendl;
    ...
    _collect_allocation_stats(need, alloc_size,
                              extents->size() - extent_count0);
...
int64_t BlueStore::_get_bluefs_size_delta(uint64_t bluefs_free,
  ...
  dout(10) << __func__ << " bluefs " << byte_u_t(bluefs_free) << " free ("
           << bluefs_free_ratio << ") bluestore " << byte_u_t(my_free)
           << " free (" << my_free_ratio << "), bluefs_ratio " << bluefs_ratio
           << dendl;
  ...
  if (bluefs_ratio < cct->_conf->bluestore_bluefs_min_ratio) {
    ...
    if (gift >= my_free)
      gift = my_free / 2;
    dout(10) << __func__ << " bluefs_ratio " << bluefs_ratio << " < min_ratio "
             << cct->_conf->bluestore_bluefs_min_ratio << ", should gift "
             << byte_u_t(gift) << dendl;
    ...
    dout(10) << __func__ << " bluefs_ratio " << bluefs_ratio << " > max_ratio "
             << cct->_conf->bluestore_bluefs_max_ratio << ", should reclaim "
  ...
  if (bluefs_total < cct->_conf->bluestore_bluefs_min &&
    ...
    dout(10) << __func__ << " bluefs_total " << bluefs_total << " < min "
             << cct->_conf->bluestore_bluefs_min << ", should gift "
             << byte_u_t(g) << dendl;
    if (g > gift)
      gift = g;
    reclaim = 0;
  ...
  if (bluefs_free < min_free && min_free < free_cap) {
    uint64_t g = min_free - bluefs_free;
    dout(10) << __func__ << " bluefs_free " << bluefs_free << " < min "
             << min_free << ", should gift " << byte_u_t(g) << dendl;
    if (g > gift)
      gift = g;
    reclaim = 0;
  }
  ...
  if (bluefs_free > max_free) {
    dout(10) << __func__ << " bluefs_free " << bluefs_free << " > max "
             << max_free << ", stop gifting for now" << dendl;
    gift = 0;
  }
...
int BlueStore::_balance_bluefs_freespace() {
  ...
  int64_t delta =
      _get_bluefs_size_delta(bluefs_usage[bluefs_layout.shared_bdev].first,
  ...
  if (delta < 0) {
    ...
    while (reclaim > 0) {
      ...
      int r =
          bluefs->reclaim_blocks(bluefs_layout.shared_bdev, reclaim, &extents);
      if (r < 0) {
        derr << __func__ << " failed to reclaim space from bluefs" << dendl;
        break;
      }
      for (auto e : extents) {
        ++out_of_sync_fm;
...
int BlueStore::_open_collections() {
  ...
  for (it->upper_bound(string()); it->valid(); it->next()) {
    ...
    if (cid.parse(it->key())) {
      ...
      dout(20) << __func__ << " opened " << cid << " " << c << " " << c->cnode
               << dendl;
      _osr_attach(c.get());
      coll_map[cid] = c;
  ...
  if (r >= 0) {
    ...
    for (it->upper_bound(string()); it->valid(); it->next()) {
      ...
      try {
        ...
        dout(30) << __func__ << " pool " << pool_id << " statfs " << st
                 << dendl;
      } catch (buffer::error &e) {
...
int BlueStore::_setup_block_symlink_or_file(string name, string epath,
                                            uint64_t size, bool create) {
  dout(20) << __func__ << " name " << name << " path " << epath << " size "
           << size << " create=" << (int)create << dendl;
  int r = 0;
  int flags = O_RDWR | O_CLOEXEC;
  if (create)
    flags |= O_CREAT;
  if (epath.length()) {
    ...
    if (!epath.compare(0, strlen(SPDK_PREFIX), SPDK_PREFIX)) {
      ...
      if (fd < 0) {
        r = -errno;
        derr << __func__ << " failed to open " << epath
             << " file: " << cpp_strerror(r) << dendl;
        return r;
      }
      ...
      dout(1) << __func__ << " created " << name << " symlink to " << epath
              << dendl;
      VOID_TEMP_FAILURE_RETRY(::close(fd));
    }
  }
  if (size) {
    ...
    if (fd >= 0) {
      ...
      struct stat st;
      int r = ::fstat(fd, &st);
      if (r == 0 && S_ISREG(st.st_mode) && // if it is a regular file
        ...
        if (cct->_conf->bluestore_block_preallocate_file) {
          ...
          if (r > 0) {
            derr << __func__ << " failed to prefallocate " << name
                 << " file to " << size << ": " << cpp_strerror(r) << dendl;
            VOID_TEMP_FAILURE_RETRY(::close(fd));
            return -r;
          }
        }
      ...
      int r = -errno;
      if (r != -ENOENT) {
        derr << __func__ << " failed to open " << name
             << " file: " << cpp_strerror(r) << dendl;
        return r;
      }
    }
...
int BlueStore::mkfs() {
...

  {
    ...
    if (r == 0) {
      if (type != "bluestore") {
        derr << __func__ << " expected bluestore, but type is " << type
             << dendl;
        return -EIO;
      }
    } else {
  ...
  if (r < 0 || old_fsid.is_zero()) {
    ...
    if (!fsid.is_zero() && fsid != old_fsid) {
      derr << __func__ << " on-disk fsid " << old_fsid << " != provided "
           << fsid << dendl;
      r = -EINVAL;
      goto out_close_fsid;
    }
  ...
  if (!isp2(min_alloc_size)) {
    derr << __func__ << " min_alloc_size 0x" << std::hex << min_alloc_size
         << std::dec << " is not power of 2 aligned!" << dendl;
    r = -EINVAL;
    goto out_close_bdev;
  }
...
int BlueStore::add_new_bluefs_device(int id, const string &dev_path) {
  ...
  if (!cct->_conf->bluestore_bluefs) {
    derr << __func__ << " bluefs isn't configured, can't add new device "
         << dendl;
    return -EIO;
  }
  ...
  if (id == BlueFS::BDEV_NEWWAL) {
    ...
    r = _setup_block_symlink_or_file(
        "block.wal", dev_path, cct->_conf->bluestore_block_wal_size, true);
    ceph_assert(r == 0);
    ...
    if (bluefs->bdev_support_label(BlueFS::BDEV_NEWWAL)) {
      r = _check_or_set_bdev_label(
          p, bluefs->get_block_device_size(BlueFS::BDEV_NEWWAL), "bluefs wal",
          true);
      ceph_assert(r == 0);
    }
    ...
    r = _setup_block_symlink_or_file("block.db", dev_path,
                                     cct->_conf->bluestore_block_db_size, true);
    ceph_assert(r == 0);
    ...
    if (bluefs->bdev_support_label(BlueFS::BDEV_NEWDB)) {
      r = _check_or_set_bdev_label(
          p, bluefs->get_block_device_size(BlueFS::BDEV_NEWDB), "bluefs db",
          true);
      ceph_assert(r == 0);
    }
  ...
  bluefs->add_block_extent(id, reserved,
...
int BlueStore::migrate_to_existing_bluefs_device(const set<int> &devs_source,
  ...
  if (id == BlueFS::BDEV_SLOW && target_free < used_space) {
    ...
    r = mount();
    ceph_assert(r == 0);
    dout(1) << __func__ << " Allocating more space at slow device for BlueFS: +"
            << used_space - target_free << " bytes" << dendl;
    r = allocate_bluefs_freespace(used_space - target_free,
                                  used_space - target_free, nullptr);

    umount();
    if (r != 0) {
      derr << __func__ << " can't migrate, unable to allocate extra space: "
           << used_space - target_free << " at target:" << id << dendl;
      return -ENOSPC;
    }
    ...
    derr << __func__ << " can't migrate, free space at target: " << target_free
         << " is less than required space: " << used_space << dendl;
    return -ENOSPC;
  }
  ...
  if (r < 0) {
    derr << __func__ << " failed during BlueFS migration, " << cpp_strerror(r)
         << dendl;
    goto shutdown;
  }
...
int BlueStore::migrate_to_new_bluefs_device(const set<int> &devs_source, int id,
  ...
  if (id == BlueFS::BDEV_NEWWAL) {
    ...
    if (bluefs->bdev_support_label(BlueFS::BDEV_NEWWAL)) {
      r = _check_or_set_bdev_label(
          dev_path, bluefs->get_block_device_size(BlueFS::BDEV_NEWWAL),
          "bluefs wal", true);
      ceph_assert(r == 0);
    }
    reserved = BDEV_LABEL_BLOCK_SIZE;
    ...
    if (bluefs->bdev_support_label(BlueFS::BDEV_NEWDB)) {
      r = _check_or_set_bdev_label(
          dev_path, bluefs->get_block_device_size(BlueFS::BDEV_NEWDB),
          "bluefs db", true);
      ceph_assert(r == 0);
    }
    reserved = SUPER_RESERVED;
  ...
  r = _setup_block_symlink_or_file(target_name, dev_path, target_size, true);
  ceph_assert(r == 0);
  dout(0) << __func__ << " success" << dendl;
...
int BlueStore::expand_devices(ostream &out) {
  ...
  for (auto devid : {BlueFS::BDEV_WAL, BlueFS::BDEV_DB}) {
    ...
    if (end < size) {
      out << devid << " : expanding "
          << " from 0x" << std::hex << end << " to 0x" << size << std::dec
          << std::endl;
      ...
      int r = _read_bdev_label(cct, path, &label);
      if (r < 0) {
        derr << "unable to read label for " << path << ": " << cpp_strerror(r)
             << dendl;
        continue;
      }
      label.size = size;
      ...
      if (r < 0) {
        derr << "unable to write label for " << path << ": " << cpp_strerror(r)
             << dendl;
        continue;
      }
      out << devid << " : size label updated to " << size << std::endl;
    }
  }
  uint64_t size0 = fm->get_size();
  uint64_t size = bdev->get_size();
  if (size0 < size) {
    out << bluefs_layout.shared_bdev << " : expanding "
        << " from 0x" << std::hex << size0 << " to 0x" << size << std::dec
        << std::endl;
    ...
    if (r < 0) {
      ...
      if (r < 0) {
        ...
        out << bluefs_layout.shared_bdev << " : size label updated to " << size
            << std::endl;
      }
    }
  }
...
int _fsck_sum_extents(const PExtentVector &extents, bool compressed,
...
int BlueStore::_fsck_check_extents(
    const coll_t &cid, const ghobject_t &oid, const PExtentVector &extents,
    bool compressed, mempool_dynamic_bitset &used_blocks, uint64_t granularity,
    BlueStoreRepairer *repairer, store_statfs_t &expected_statfs,
    FSCKDepth depth) {
    ...
    if (depth != FSCK_SHALLOW) {
      ...
      apply_for_bitset_range(
          ...
          [&](uint64_t pos, mempool_dynamic_bitset &bs) {
            if (bs.test(pos)) {
              if (repairer) {
                repairer->note_misreference(pos * min_alloc_size,
                                            min_alloc_size, !already);
              }
              if (!already) {
                derr << "fsck error: " << oid << " extent " << e
                     << " or a subset is already allocated (misreferenced)"
                     << dendl;
                ++errors;
                already = true;
              }
      ...
      if (repairer) {
        repairer->get_space_usage_tracker().set_used(e.offset, e.length, cid,
                                                     oid);
      }
...
void BlueStore::_fsck_check_pool_statfs(
    BlueStore::per_pool_statfs &expected_pool_statfs, int64_t &errors,
    ...
    for (it->lower_bound(string()); it->valid(); it->next()) {
      ...
      if (key == BLUESTORE_GLOBAL_STATFS_KEY) {
        if (repairer) {
          ...
          derr << "fsck error: "
               << "legacy statfs record found, removing" << dendl;
        }
        continue;
      }
      uint64_t pool_id;
      ...
      try {
        ...
        if (repairer) {
          dout(20) << __func__ << " undecodable Pool StatFS record, key:'"
                   << pretty_binary_string(key) << "', removing" << dendl;
          repairer->remove_key(db, PREFIX_STAT, key);
        }
        ++errors;
        vstatfs.reset();
      ...
      if (stat_it == expected_pool_statfs.end()) {
        if (vstatfs.is_empty()) {
          ...
          dout(20)
              << "fsck inf: found empty stray Pool StatFS record for pool id 0x"
      ...
      if (!(stat_it->second == statfs)) {
        derr << "fsck error: actual " << statfs << " != expected "
             << stat_it->second << " for pool " << std::hex << pool_id
             << std::dec << dendl;
        if (repairer) {
  ...
  for (auto &s : expected_pool_statfs) {
    ...
    derr << "fsck error: missing Pool StatFS record for pool " << std::hex
         << s.first << std::dec << dendl;
    if (repairer) {
      string key;
  ...
  if (!per_pool_stat_collection && repairer) {
...
BlueStore::OnodeRef BlueStore::fsck_check_objects_shallow(
    BlueStore::FSCKDepth depth, int64_t pool_id, BlueStore::CollectionRef c,
    const ghobject_t &oid, const string &key, const bufferlist &value,
  ...
  // lextents
  uint64_t pos = 0;
  mempool::bluestore_fsck::map<BlobRef, bluestore_blob_use_tracker_t> ref_map;
  for (auto &l : o->extent_map.extent_map) {
    dout(20) << __func__ << "    " << l << dendl;
    if (l.logical_offset < pos) {
      derr << "fsck error: " << oid << " lextent at 0x" << std::hex
           << l.logical_offset
           << " overlaps with the previous, which ends at 0x" << pos << std::dec
           << dendl;
      ++errors;
    }
    if (depth != FSCK_SHALLOW &&
      ...
      derr << "fsck error: " << oid << " lextent at 0x" << std::hex
           << l.logical_offset << "~" << l.length << " spans a shard boundary"
           << std::dec << dendl;
      ++errors;
    }
    ...
    ref.get(l.blob_offset, l.length);
    ++num_extents;
  ...
  for (auto &i : ref_map) {
    ...
    if (blob.is_compressed()) {
      ...
      res_statfs->data_compressed_original += i.first->get_referenced_bytes();
    }
    if (blob.is_shared()) {
      if (i.first->shared_blob->get_sbid() > blobid_max) {
        derr << "fsck error: " << oid << " blob " << blob << " sbid "
             << i.first->shared_blob->get_sbid() << " > blobid_max "
             << blobid_max << dendl;
        ++errors;
      } else if (i.first->shared_blob->get_sbid() == 0) {
        derr << "fsck error: " << oid << " blob " << blob
             << " marked as shared but has uninitialized sbid" << dendl;
        ++errors;
      }
      ...
      errors += _fsck_check_extents(
          c->cid, oid, blob.get_extents(), blob.is_compressed(), *used_blocks,
          fm->get_alloc_size(), repairer, *res_statfs, depth);
    } else {
      errors += _fsck_sum_extents(blob.get_extents(), blob.is_compressed(),
                                  *res_statfs);
    }
  } // for (auto& i : ref_map)
...
public:
  ShallowFSCKThreadPool(CephContext *cct_, std::string nm, std::string tn,
                        int n)
  ...
  void worker(ThreadPool::WorkThread *wt) override {
    ...
    while (!_stop) {
      ...
      if (item) {
        processing++;
        TPHandle tp_handle(cct, nullptr, wq->timeout_interval,
                           wq->suicide_interval);
  ...
  struct FSCKWorkQueue : public ThreadPool::WorkQueue_ {
    ...
    FSCKWorkQueue(std::string n, size_t _batchCount, BlueStore *_store,
                  ceph::mutex *_sb_info_lock,
                  BlueStore::sb_info_map_t &_sb_info,
                  BlueStoreRepairer *_repairer)
        : WorkQueue_(n, time_t(), time_t()), batchCount(_batchCount),
          store(_store), sb_info_lock(_sb_info_lock), sb_info(&_sb_info),
          repairer(_repairer) {
      batches = new Batch[batchCount];
    }
    ~FSCKWorkQueue() { delete[] batches; }
    ...
    bool _empty() override { ceph_assert(false); }
    ...
    void _void_process(void *item, TPHandle &handle) override {
      ...
      BlueStore::FSCK_ObjectCtx ctx(
          batch->errors, batch->warnings, batch->num_objects,
          batch->num_extents, batch->num_blobs, batch->num_sharded_objects,
          batch->num_spanning_blobs,
          nullptr, // used_blocks
          nullptr, // used_omap_head
          sb_info_lock, *sb_info, batch->expected_store_statfs,
          batch->expected_pool_statfs, repairer);

      for (size_t i = 0; i < batch->entry_count; i++) {
        auto &entry = batch->entries[i];

        store->fsck_check_objects_shallow(
            BlueStore::FSCK_SHALLOW, entry.pool_id, entry.c, entry.oid,
     ...
     * This function is called after _void_process with the global thread pool
     * lock held, so at most one copy will execute simultaneously for a given
     * thread pool. It can be used for non-thread-safe finalization. */
    void _void_process_finish(void *) override { ceph_assert(false); }

    bool queue(int64_t pool_id, BlueStore::CollectionRef c,
               const ghobject_t &oid, const string &key,
    ...
    void finalize(ThreadPool &tp, BlueStore::FSCK_ObjectCtx &ctx) {
      if (batch_acquired) {
        auto &batch = batches[last_batch_pos];
        ceph_assert(batch.running);
        batch.running.fetch_sub(1);
      ...
      for (size_t i = 0; i < batchCount; i++) {
        ...
        if (batch.entry_count) {
          TPHandle tp_handle(store->cct, nullptr, timeout_interval,
                             suicide_interval);
        ...
        for (auto it = batch.expected_pool_statfs.begin();
             it != batch.expected_pool_statfs.end(); it++) {
...
void BlueStore::_fsck_check_object_omap(FSCKDepth depth, OnodeRef &o,
                                        const BlueStore::FSCK_ObjectCtx &ctx) {
  auto &errors = ctx.errors;
  ...
  if (!o->onode.is_perpool_omap() && !o->onode.is_pgmeta_omap()) {
    if (per_pool_omap) {
      fsck_derr(errors, MAX_FSCK_ERROR_LINES)
          << "fsck error: " << o->oid
          << " has omap that is not per-pool or pgmeta" << fsck_dendl;
      ++errors;
    } else {
      const char *w;
      int64_t num;
  ...
  if (repairer && !o->onode.is_perpool_omap() && !o->onode.is_pgmeta_omap()) {
...
void BlueStore::_fsck_check_objects(FSCKDepth depth,
  ...
  if (it) {
    ...
    std::unique_ptr<WQ> wq(new WQ("FSCKWorkQueue", (thread_count ?: 1) * 32,
                                  this, sb_info_lock, sb_info, repairer));

    ShallowFSCKThreadPool thread_pool(cct, "ShallowFSCKThreadPool",
                                      "ShallowFSCK", thread_count);
    ...
    for (it->lower_bound(string()); it->valid(); it->next()) {
      ...
      if (is_extent_shard_key(it->key())) {
        ...
        derr << "fsck error: stray shard 0x" << std::hex << offset << std::dec
             << dendl;
        if (expecting_shards.empty()) {
      ...
      if (r < 0) {
        derr << "fsck error: bad object key " << pretty_binary_string(it->key())
             << dendl;
        ++errors;
        continue;
      }
      if (!c || oid.shard_id != pgid.shard ||
      ...
      if (depth != FSCK_SHALLOW && !expecting_shards.empty()) {
        for (auto &k : expecting_shards) {
          derr << "fsck error: missing shard key " << pretty_binary_string(k)
               << dendl;
        }
        ++errors;
      ...
      bool queued = false;
      if (depth == FSCK_SHALLOW && thread_count > 0) {
        queued = wq->queue(pool_id, c, oid, it->key(), it->value());
      }
      OnodeRef o;
      map<BlobRef, bluestore_blob_t::unused_t> referenced;
      ...
      if (!queued) {
        ...
        o = fsck_check_objects_shallow(depth, pool_id, c, oid, it->key(),
                                       it->value(), &expecting_shards,
                                       &referenced, ctx);
      }
      ...
      if (depth != FSCK_SHALLOW) {
        ...
        for (auto &i : referenced) {
          ...
          if (i.second & blob.unused) {
            derr << "fsck error: " << oid << " blob claims unused 0x"
                 << std::hex << blob.unused << " but extents reference 0x"
                 << i.second << std::dec << " on blob " << *i.first << dendl;
            ++errors;
          }
          if (blob.has_csum()) {
            uint64_t blob_len = blob.get_logical_length();
            ...
            for (unsigned p = 0; p < csum_count; ++p) {
              ...
              unsigned lastbit =
                  (pos + csum_chunk_size - 1) / unused_chunk_size;
              ...
              if ((blob.unused & mask) == mask) {
                ...
                if (blob.get_csum_item(p) != 0) {
                  derr << "fsck error: " << oid << " blob claims csum chunk 0x"
                       << std::hex << pos << "~" << csum_chunk_size
        ...
        if (depth == FSCK_DEEP) {
          ...
          do {
            uint64_t l =
                std::min(uint64_t(o->onode.size - offset), max_read_block);
    ...
    if (depth == FSCK_SHALLOW && thread_count > 0) {
      ...
      if (processed_myself) {
        ...
        dout(0) << __func__ << " partial offload"
                << ", done myself " << processed_myself << " of "
                << ctx.num_objects << "objects, threads " << thread_count
                << dendl;
      }
    }
  } // if (it)
...
int BlueStore::_fsck(BlueStore::FSCKDepth depth, bool repair) {
  dout(1) << __func__ << (repair ? " repair" : " check")
          << (depth == FSCK_DEEP      ? " (deep)"
              : depth == FSCK_SHALLOW ? " (shallow)"
                                      : " (regular)")
          << dendl;
...
int BlueStore::_fsck_on_open(BlueStore::FSCKDepth depth, bool repair) {
  dout(1) << __func__ << " <<<START>>>" << (repair ? " repair" : " check")
          << (depth == FSCK_DEEP      ? " (deep)"
              : depth == FSCK_SHALLOW ? " (shallow)"
                                      : " (regular)")
          << " start" << dendl;
  ...
  apply_for_bitset_range(
      0, std::max<uint64_t>(min_alloc_size, SUPER_RESERVED),
      fm->get_alloc_size(), used_blocks,
      [&](uint64_t pos, mempool_dynamic_bitset &bs) { bs.set(pos); });
  if (repair) {
    repairer.get_space_usage_tracker().init(bdev->get_size(), min_alloc_size);
  }
      ...
      try {
        ...
        if (!bluefs_extents_db.empty()) {
          derr << "fsck error: bluefs_extents inconsistency, "
               << "downgrade to previous releases might be broken." << dendl;
          ++errors;
        }
      } catch (buffer::error &e) {
        derr << "fsck error: failed to retrieve bluefs_extents from kv"
             << dendl;
        ++errors;
      }
  ...
  // walk PREFIX_OBJ
  {
    ...
    BlueStore::FSCK_ObjectCtx ctx(
        errors, warnings, num_objects, num_extents, num_blobs,
        num_sharded_objects, num_spanning_blobs, &used_blocks, &used_omap_head,
        // no need for the below lock when in non-shallow mode as
        // there is no multithreading in this case
        depth == FSCK_SHALLOW ? &sb_info_lock : nullptr, sb_info,
        expected_store_statfs, expected_pool_statfs,
  ...
  if (it) {
    ...
    for (it->lower_bound(string()); it->valid(); it->next()) {
      ...
      if (get_key_shared_blob(key, &sbid)) {
        derr << "fsck error: bad key '" << key << "' in shared blob namespace"
             << dendl;
        if (repair) {
          repairer.remove_key(db, PREFIX_SHARED_BLOB, key);
        }
      ...
      if (p == sb_info.end()) {
        ...
        try {
          ...
          if (repair) {
            dout(20) << __func__ << " undecodable Shared Blob, key:'"
                     << pretty_binary_string(it->key()) << "', removing"
                     << dendl;
        ...
        if (shared_blob.ref_map != sbi.ref_map) {
          derr << "fsck error: shared blob 0x" << std::hex << sbid << std::dec
        ...
        errors += _fsck_check_extents(
            sbi.cid, p->second.oids.front(), extents, p->second.compressed,
            used_blocks, fm->get_alloc_size(), repair ? &repairer : nullptr,
            *expected_statfs, depth);
        sbi.passed = true;
      }
    }
  } // if (it)
  ...
  if (repair && repairer.preprocess_misreference(db)) {
    ...
    if (it) {
      ...
      for (it->lower_bound(string()); it->valid() && !bypass_rest; it->next()) {
        dout(30) << __func__ << " key " << pretty_binary_string(it->key())
                 << dendl;
        if (is_extent_shard_key(it->key())) {
          continue;
        ...
        for (auto b : blobs) {
          ...
          dout(10) << __func__ << " fix misreferences in oid:" << oid << " "
                   << *b << dendl;
          uint64_t b_off = 0;
          ...
          for (auto e = pextents.begin(); e != pextents.end();
            ...
            PExtentVector exts;
            int64_t alloc_len =
                alloc->allocate(e->length, min_alloc_size, 0, 0, &exts);
            if (alloc_len < 0 || alloc_len < (int64_t)e->length) {
              derr << __func__ << " failed to allocate 0x" << std::hex
                   << e->length << " allocated 0x "
                   << (alloc_len < 0 ? 0 : alloc_len) << " min_alloc_size 0x"
                   << min_alloc_size << " available 0x " << alloc->get_free()
                   << std::dec << dendl;
              if (alloc_len > 0) {
                alloc->release(exts);
              }
            ...
            if (r < 0) {
              derr << __func__ << " failed to read from 0x" << std::hex
                   << e->offset << "~" << e->length << std::dec << dendl;
              ceph_abort_msg("read failed, wtf");
            }
            pext_to_release.push_back(*e);
            e = pextents.erase(e);
            e = pextents.insert(e, exts.begin(), exts.end());
            b->get_blob().map_bl(b_off, bl,
    ...
    for (auto &p : sb_info) {
      ...
      if (repair && (!sbi.passed || sbi.updated)) {
        ...
        if (sbi.ref_map.empty()) {
          ...
          dout(20) << __func__ << " " << *sbi.sb << " is empty, removing"
                   << dendl;
          ...
          dout(20) << __func__ << " " << *sbi.sb << " is " << bl.length()
                   << " bytes, updating" << dendl;
  ...
  if (!per_pool_stat_collection && !repair &&
    ...
    derr << "fsck error: actual " << actual_statfs << " != expected "
         << expected_store_statfs << dendl;
    if (repair) {
  ...
  _fsck_check_pool_statfs(expected_pool_statfs, errors, warnings,
                          repair ? &repairer : nullptr);
  ...
  if (depth != FSCK_SHALLOW) {
    ...
    if (it) {
      ...
      for (it->lower_bound(string()); it->valid(); it->next()) {
        ...
        if (used_omap_head.count(omap_head) == 0 &&
          ...
          fsck_derr(errors, MAX_FSCK_ERROR_LINES)
              << "fsck error: found stray omap data on omap_head " << omap_head
              << " " << last_omap_head << " " << used_omap_head.count(omap_head)
              << fsck_dendl;
          ++errors;
      ...
      for (it->lower_bound(string()); it->valid(); it->next()) {
        ...
        if (used_omap_head.count(omap_head) == 0 &&
          ...
          fsck_derr(errors, MAX_FSCK_ERROR_LINES)
              ...
              << omap_head << " " << last_omap_head << " "
              << used_omap_head.count(omap_head) << fsck_dendl;
          last_omap_head = omap_head;
          ++errors;
        }
      }
    ...
    if (it) {
      for (it->lower_bound(string()); it->valid(); it->next()) {
        ...
        dout(20) << __func__ << "  deferred " << wt.seq << " ops "
                 << wt.ops.size() << " released 0x" << std::hex << wt.released
                 << std::dec << dendl;
    ...
    dout(1) << __func__ << " checking freelist vs allocated" << dendl;
    {
      ...
      for (auto e = bluefs_extents.begin(); e != bluefs_extents.end(); ++e) {
        apply_for_bitset_range(
            e.get_start(), e.get_len(), fm->get_alloc_size(), used_blocks,
            [&](uint64_t pos, mempool_dynamic_bitset &bs) { bs.reset(pos); });
      }
      fm->enumerate_reset();
      uint64_t offset, length;
      while (fm->enumerate_next(db, &offset, &length)) {
            ...
            [&](uint64_t pos, mempool_dynamic_bitset &bs) {
              if (bs.test(pos)) {
                if (offset == SUPER_RESERVED &&
                  ...
                  // this is due to the change just after luminous to
                  // min_alloc_size granularity allocations, and our baked in
                  // assumption at the top of _fsck that
                  // 0~round_up_to(SUPER_RESERVED,min_alloc_size) is used (vs
                  // luminous's round_up_to(SUPER_RESERVED,block_size)).
                  // harmless, since we will never allocate this region below
                  // min_alloc_size.
                  dout(10) << __func__
                  ...
                  intersects = true;
                  if (repair) {
                    repairer.fix_false_free(db, fm, pos * min_alloc_size,
                                            min_alloc_size);
                  }
                }
              } else {
        ...
        if (intersects) {
          derr << "fsck error: free extent 0x" << std::hex << offset << "~"
               << length << std::dec << " intersects allocated blocks" << dendl;
          ++errors;
        }
      }
      fm->enumerate_reset();
          ...
          while (true) {
            ...
            if (next != cur + 1) {
              ...
              derr << "fsck error: leaked extent 0x" << std::hex
                   ...
                   << dendl;
              if (repair) {
                repairer.fix_leaked(db, fm, start * min_alloc_size,
...
out_scan:
  dout(2) << __func__ << " " << num_objects << " objects, "
          << num_sharded_objects << " of them sharded.  " << dendl;
  dout(2) << __func__ << " " << num_extents << " extents to " << num_blobs
          << " blobs, " << num_spanning_blobs << " spanning, "
          << num_shared_blobs << " shared." << dendl;

  utime_t duration = ceph_clock_now() - start;
  dout(1) << __func__ << " <<<FINISH>>> with " << errors << " errors, "
          << warnings << " warnings, " << repaired << " repaired, "
          << (errors + warnings - (int)repaired) << " remaining in " << duration
          << " seconds" << dendl;
...
void BlueStore::inject_leaked(uint64_t len) {
  ...
  PExtentVector exts;
  int64_t alloc_len =
      alloc->allocate(len, min_alloc_size, min_alloc_size * 256, 0, &exts);
...
void BlueStore::inject_legacy_omap(coll_t cid, ghobject_t oid) {
  ...
  o->onode.clear_flag(bluestore_onode_t::FLAG_PERPOOL_OMAP |
...
void BlueStore::inject_statfs(const string &key,
...
void BlueStore::inject_misreference(coll_t cid1, ghobject_t oid1, coll_t cid2,
                                    ghobject_t oid2, uint64_t offset) {
  OnodeRef o1;
...
void BlueStore::collect_metadata(map<string, string> *pm) {
  ...
  if (bluefs) {
    ...
    (*pm)["bluefs_dedicated_db"] = stringify((int)bluefs_layout.dedicated_db);
    (*pm)["bluefs_dedicated_wal"] = stringify((int)bluefs_layout.dedicated_wal);
...
int BlueStore::get_numa_node(int *final_node, set<int> *out_nodes,
...
void BlueStore::_get_statfs_overall(struct store_statfs_t *buf) {
  ...
  if (bluefs) {
    ...
    int64_t shared_available = std::min(
        bluefs_free, int64_t(bluefs_total - cct->_conf->bluestore_bluefs_min));
    ...
    buf->internal_metadata =
        std::max(bluefs->get_used(),
                 (uint64_t)cct->_conf->bluestore_bluefs_min) -
        buf->omap_allocated;
  }
...
int BlueStore::statfs(struct store_statfs_t *buf, osd_alert_list_t *alerts) {
  if (alerts) {
    alerts->clear();
    _log_alerts(*alerts);
  }
...
int BlueStore::pool_statfs(uint64_t pool_id, struct store_statfs_t *buf,
  ...
  buf->omap_allocated =
      db->estimate_prefix_size(PREFIX_PERPOOL_OMAP, key_prefix);
...
void BlueStore::_check_no_per_pool_omap_alert() {
  string s;
  if (!per_pool_omap && cct->_conf->bluestore_warn_on_no_per_pool_omap) {
...
void BlueStore::_update_cache_logger() {
  ...
  for (auto c : buffer_cache_shards) {
    c->add_stats(&num_extents, &num_blobs, &num_buffers, &num_buffer_bytes);
  }
...
BlueStore::create_new_collection(const coll_t &cid) {
  std::unique_lock l{coll_lock};
  auto c = ceph::make_ref<Collection>(
      this, onode_cache_shards[cid.hash_to_shard(onode_cache_shards.size())],
      buffer_cache_shards[cid.hash_to_shard(buffer_cache_shards.size())], cid);
  new_coll_map[cid] = c;
  _osr_attach(c.get());
  return c;
}

void BlueStore::set_collection_commit_queue(const coll_t &cid,
...
int BlueStore::stat(CollectionHandle &c_, const ghobject_t &oid,
...
int BlueStore::set_collection_opts(CollectionHandle &ch,
...
int BlueStore::read(CollectionHandle &c_, const ghobject_t &oid,
  ...
  int r;
  {
    ...
    auto start1 = mono_clock::now();
    OnodeRef o = c->get_onode(oid, false);
    log_latency("get_onode@read", l_bluestore_read_onode_meta_lat,
                mono_clock::now() - start1, cct->_conf->bluestore_log_op_age);
    if (!o || !o->exists) {
      r = -ENOENT;
      goto out;
    }
...
out:
  ...
  dout(10) << __func__ << " " << cid << " " << oid << " 0x" << std::hex
           << offset << "~" << length << std::dec << " = " << r << dendl;
  log_latency(__func__, l_bluestore_read_lat, mono_clock::now() - start,
              cct->_conf->bluestore_log_op_age);
  return r;
}

void BlueStore::_read_cache(OnodeRef o, uint64_t offset, size_t length,
  ...
  while (left > 0 && lp != o->extent_map.extent_map.end()) {
    ...
    bptr->shared_blob->bc.read(bptr->shared_blob->get_cache(), b_off, b_len,
                               cache_res, cache_interval, read_cache_policy);
    dout(20) << __func__ << "  blob " << *bptr << std::hex << " need 0x"
             << b_off << "~" << b_len << " cache has 0x" << cache_interval
             << std::dec << dendl;
    ...
    while (b_len > 0) {
      unsigned l;
      if (pc != cache_res.end() && pc->first == b_off) {
        l = pc->second.length();
...
int BlueStore::_prepare_read_ioc(blobs2read_t &blobs2read,
                                 vector<bufferlist> *compressed_blob_bls,
                                 IOContext *ioc) {
  for (auto &p : blobs2read) {
    const BlobRef &bptr = p.first;
    regions2read_t &r2r = p.second;
    dout(20) << __func__ << "  blob " << *bptr << std::hex << " need " << r2r
             << std::dec << dendl;
    if (bptr->get_blob().is_compressed()) {
      ...
      auto r = bptr->get_blob().map(0, bptr->get_blob().get_ondisk_length(),
                                    [&](uint64_t offset, uint64_t length) {
                                      int r = bdev->aio_read(offset, length,
                                                             &bl, ioc);
                                      if (r < 0)
                                        return r;
                                      return 0;
      ...
      for (auto &req : r2r) {
        dout(20) << __func__ << "    region 0x" << std::hex
                 << req.regs.front().logical_offset << ": 0x"
                 << req.regs.front().blob_xoffset << " reading 0x" << req.r_off
                 << "~" << req.r_len << std::dec << dendl;

        // read it
        auto r = bptr->get_blob().map(
            req.r_off, req.r_len, [&](uint64_t offset, uint64_t length) {
...
int BlueStore::_generate_read_result_bl(OnodeRef o, uint64_t offset,
                                        size_t length,
                                        ready_regions_t &ready_regions,
                                        vector<bufferlist> &compressed_blob_bls,
                                        blobs2read_t &blobs2read, bool buffered,
                                        bool *csum_error, bufferlist &bl) {
  ...
  while (b2r_it != blobs2read.end()) {
    ...
    dout(20) << __func__ << "  blob " << *bptr << std::hex << " need 0x" << r2r
             << std::dec << dendl;
    if (bptr->get_blob().is_compressed()) {
      ...
      for (auto &req : r2r) {
        for (auto &r : req.regs) {
          ready_regions[r.logical_offset].substr_of(raw_bl, r.blob_xoffset,
                                                    r.length);
        }
      }
    } else {
  ...
  while (pos < length) {
    if (pr != pr_end && pr->first == pos + offset) {
      ...
      dout(30) << __func__ << " assemble 0x" << std::hex << pos
               << ": zeros for 0x" << (pos + offset) << "~" << l << std::dec
               << dendl;
      bl.append_zero(l);
      pos += l;
    }
...
int BlueStore::_do_read(Collection *c, OnodeRef o, uint64_t offset,
  ...
  dout(20) << __func__ << " 0x" << std::hex << offset << "~" << length
           << " size 0x" << o->onode.size << " (" << std::dec << o->onode.size
           << ")" << dendl;
  bl.clear();
  ...
  log_latency(__func__, l_bluestore_read_onode_meta_lat,
  ...
  if (retry_count) {
    ...
    dout(5) << __func__ << " read at 0x" << std::hex << offset << "~" << length
            << " failed " << std::dec << retry_count
            << " times before succeeding" << dendl;
  }
  return r;
}

int BlueStore::_verify_csum(OnodeRef &o, const bluestore_blob_t *blob,
                            uint64_t blob_xoffset, const bufferlist &bl,
  ...
  if (cct->_conf->bluestore_debug_inject_csum_err_probability > 0 &&
      (rand() % 10000) <
          cct->_conf->bluestore_debug_inject_csum_err_probability * 10000.0) {
    derr << __func__ << " injecting bluestore checksum verifcation error"
         << dendl;
    bad = blob_xoffset;
    r = -1;
    bad_csum = 0xDEADBEEF;
  ...
  if (r < 0) {
    if (r == -1) {
      ...
      derr << __func__ << " bad "
           << Checksummer::get_csum_type_string(blob->csum_type) << "/0x"
           << std::hex << blob->get_csum_chunk_size()
           << " checksum at blob offset 0x" << bad << ", got 0x" << bad_csum
           << ", expected 0x"
           << blob->get_csum_item(bad / blob->get_csum_chunk_size()) << std::dec
           << ", device location " << pex << ", logical extent 0x" << std::hex
           << (logical_offset + bad - blob_xoffset) << "~"
           << blob->get_csum_chunk_size() << std::dec << ", object " << o->oid
           << dendl;
    } else {
  ...
  log_latency(__func__, l_bluestore_csum_lat, mono_clock::now() - start,
              cct->_conf->bluestore_log_op_age);
...
int BlueStore::_decompress(bufferlist &source, bufferlist *result) {
  ...
  log_latency(__func__, l_bluestore_decompress_lat, mono_clock::now() - start,
              cct->_conf->bluestore_log_op_age);
  return r;
}
...
// use it internally
int BlueStore::_fiemap(CollectionHandle &c_, const ghobject_t &oid,
...
int BlueStore::fiemap(CollectionHandle &c_, const ghobject_t &oid,
                      uint64_t offset, size_t length, bufferlist &bl) {
...
int BlueStore::readv(CollectionHandle &c_, const ghobject_t &oid,
                     interval_set<uint64_t> &m, bufferlist &bl,
                     uint32_t op_flags) {
  auto start = mono_clock::now();
  Collection *c = static_cast<Collection *>(c_.get());
  const coll_t &cid = c->get_cid();
...
out:
  ...
  dout(10) << __func__ << " " << cid << " " << oid << " fiemap " << m
           << std::dec << " = " << r << dendl;
  log_latency(__func__, l_bluestore_read_lat, mono_clock::now() - start,
              cct->_conf->bluestore_log_op_age);
  return r;
...
int BlueStore::_do_readv(Collection *c, OnodeRef o,
                         const interval_set<uint64_t> &m, bufferlist &bl,
                         uint32_t op_flags, uint64_t retry_count) {
  FUNCTRACE(cct);
  int r = 0;
  int read_cache_policy = 0; // do not bypass clean or dirty cache

  dout(20) << __func__ << " fiemap " << m << std::hex << " size 0x"
  ...
  auto start = mono_clock::now();
  o->extent_map.fault_range(db, m.range_start(),
                            m.range_end() - m.range_start());
  log_latency(__func__, l_bluestore_read_onode_meta_lat,
              mono_clock::now() - start, cct->_conf->bluestore_log_op_age);
  _dump_onode<30>(cct, *o);

  IOContext ioc(cct, NULL, true); // allow EIO
  vector<std::tuple<ready_regions_t, vector<bufferlist>, blobs2read_t>>
      raw_results;
  ...
  for (auto p = m.begin(); p != m.end(); p++, i++) {
    ...
    r = _prepare_read_ioc(std::get<2>(raw_results[i]),
                          &std::get<1>(raw_results[i]), &ioc);
    // we always issue aio for reading, so errors other than EIO are not allowed
    if (r < 0)
      return r;
  }
  ...
  for (auto p = m.begin(); p != m.end(); p++, i++) {
    ...
    r = _generate_read_result_bl(
        o, p.get_start(), p.get_len(), std::get<0>(raw_results[i]),
        std::get<1>(raw_results[i]), std::get<2>(raw_results[i]), buffered,
        &csum_error, t);
    if (csum_error) {
      // Handles spurious read errors caused by a kernel bug.
      // We sometimes get all-zero pages as a result of the read under
  ...
  if (retry_count) {
    logger->inc(l_bluestore_reads_with_retries);
    dout(5) << __func__ << " read fiemap " << m << " failed " << retry_count
            << " times before succeeding" << dendl;
  }
  return bl.length();
}

int BlueStore::dump_onode(CollectionHandle &c_, const ghobject_t &oid,
                          const string &section_name, Formatter *f) {
  Collection *c = static_cast<Collection *>(c_.get());
...
int BlueStore::getattr(CollectionHandle &c_, const ghobject_t &oid,
...
int BlueStore::getattrs(CollectionHandle &c_, const ghobject_t &oid,
...
int BlueStore::collection_list(CollectionHandle &c_, const ghobject_t &start,
                               const ghobject_t &end, int max,
                               vector<ghobject_t> *ls, ghobject_t *pnext) {
  Collection *c = static_cast<Collection *>(c_.get());
  c->flush();
  dout(15) << __func__ << " " << c->cid << " start " << start << " end " << end
           << " max " << max << dendl;
  int r;
  {
    std::shared_lock l(c->lock);
  ...
  dout(10) << __func__ << " " << c->cid << " start " << start << " end " << end
           << " max " << max << " = " << r << ", ls.size() = " << ls->size()
           << ", next = " << (pnext ? *pnext : ghobject_t()) << dendl;
  return r;
}

int BlueStore::_collection_list(Collection *c, const ghobject_t &start,
                                const ghobject_t &end, int max,
                                vector<ghobject_t> *ls, ghobject_t *pnext) {
  ...
  get_coll_key_range(c->cid, c->cnode.bits, &temp_start_key, &temp_end_key,
                     &start_key, &end_key);
  dout(20) << __func__ << " range " << pretty_binary_string(temp_start_key)
           << " to " << pretty_binary_string(temp_end_key) << " and "
           << pretty_binary_string(start_key) << " to "
           << pretty_binary_string(end_key) << " start " << start << dendl;
  it = db->get_iterator(PREFIX_OBJ);
...
out:
  ...
  log_latency_fn(__func__, l_bluestore_clist_lat,
                 mono_clock::now() - start_time,
                 cct->_conf->bluestore_log_collection_list_age,
                 [&](const ceph::timespan &lat) {
                   ostringstream ostr;
                   ostr << ", lat = " << timespan_str(lat) << " cid =" << c->cid
                        << " start " << start << " end " << end << " max "
                        << max;
                   return ostr.str();
                 });
  return r;
...
BlueStore::get_omap_iterator(CollectionHandle &c_, ///< [in] collection
...
uint64_t BlueStore::_get_ondisk_reserved() const {
  return round_up_to(std::max<uint64_t>(SUPER_RESERVED, min_alloc_size),
                     min_alloc_size);
}

void BlueStore::_prepare_ondisk_format_super(KeyValueDB::Transaction &t) {
  dout(10) << __func__ << " ondisk_format " << ondisk_format
           << " min_compat_ondisk_format " << min_compat_ondisk_format << dendl;
...
int BlueStore::_open_super_meta() {
  ...
  int32_t compat_ondisk_format = 0;
  {
    ...
    dout(10) << __func__ << " ondisk_format " << ondisk_format
             << " compat_ondisk_format " << compat_ondisk_format << dendl;
  }

  if (latest_ondisk_format < compat_ondisk_format) {
    derr << __func__ << " compat_ondisk_format is " << compat_ondisk_format
         << " but we only understand version " << latest_ondisk_format << dendl;
    return -EPERM;
  }
...
void BlueStore::get_db_statistics(Formatter *f) { db->get_statistics(f); }

BlueStore::TransContext *BlueStore::_txc_create(Collection *c, OpSequencer *osr,
                                                list<Context *> *on_commits) {
  TransContext *txc = new TransContext(cct, c, osr, on_commits);
  txc->t = db->get_transaction();
  osr->queue_new(txc);
  dout(20) << __func__ << " osr " << osr << " = " << txc << " seq " << txc->seq
           << dendl;
  return txc;
}
...
void BlueStore::_txc_calc_cost(TransContext *txc) {
  ...
  txc->ios = ios;
  dout(10) << __func__ << " " << txc << " cost " << txc->cost << " (" << ios
           << " ios * " << cost << " + " << txc->bytes << " bytes)" << dendl;
}
...
void BlueStore::_txc_update_store_statfs(TransContext *txc) {
  ...
  logger->inc(l_bluestore_compressed_allocated,
              txc->statfs_delta.compressed_allocated());
  logger->inc(l_bluestore_compressed_original,
              txc->statfs_delta.compressed_original());
...
void BlueStore::_txc_state_proc(TransContext *txc) {
  while (true) {
    dout(10) << __func__ << " txc " << txc << " " << txc->get_state_name()
             << dendl;
    switch (txc->state) {
...
void BlueStore::_txc_write_nodes(TransContext *txc, KeyValueDB::Transaction t) {
  dout(20) << __func__ << " txc " << txc << " onodes " << txc->onodes
           << " shared_blobs " << txc->shared_blobs << dendl;
  ...
  for (auto sb : txc->shared_blobs) {
    ...
    if (sb->persistent->empty()) {
      dout(20) << __func__ << " shared_blob 0x" << std::hex << sbid << std::dec
               << " is empty" << dendl;
...
void BlueStore::BSPerfTracker::update_from_perfcounters(PerfCounters &logger) {
  os_commit_latency_ns.consume_next(logger.get_tavg_ns(l_bluestore_commit_lat));
  os_apply_latency_ns.consume_next(logger.get_tavg_ns(l_bluestore_commit_lat));
}

void BlueStore::_txc_finalize_kv(TransContext *txc, KeyValueDB::Transaction t) {
  dout(20) << __func__ << " txc " << txc << std::hex << " allocated 0x"
           << txc->allocated << " released 0x" << txc->released << std::dec
           << dendl;
  ...
  if (!txc->allocated.empty() && !txc->released.empty()) {
    ...
    if (!overlap.empty()) {
      ...
      dout(20) << __func__ << "  overlap 0x" << std::hex << overlap
               << ", new allocated 0x" << tmp_allocated << " released 0x"
               << tmp_released << std::dec << dendl;
...
#if defined(WITH_LTTNG)
    if (txc->tracing) {
      tracepoint(bluestore, transaction_kv_submit_latency,
                 txc->osr->get_sequencer_id(), txc->seq,
...
void BlueStore::_txc_committed_kv(TransContext *txc) {
  ...
  log_latency_fn(__func__, l_bluestore_commit_lat,
                 mono_clock::now() - txc->start,
                 cct->_conf->bluestore_log_op_age,
                 [&](auto lat) { return ", txc = " + stringify(txc); });
}
...
void BlueStore::_txc_release_alloc(TransContext *txc) {
  ...
  if (likely(!cct->_conf->bluestore_debug_no_reuse_blocks)) {
    ...
    dout(10) << __func__ << "(sync) " << txc << " " << std::hex << txc->released
             << std::dec << dendl;
...

  {
    ...
    for (auto &osr : zombies) {
      if (zombie_osr_set.erase(osr->cid)) {
        ...
        dout(10) << __func__ << " empty zombie osr " << osr << " resurrected"
                 << dendl;
      }
    }
  }
  ...
  while (true) {
    ...
    if (kv_queue.empty() &&
      ...
      dout(20) << __func__ << " committing " << kv_queue.size()
               ...
               << " deferred done " << deferred_done_queue.size() << " stable "
               << deferred_stable_queue.size() << dendl;
      ...
      if (bluefs && bluefs_layout.single_shared_device()) {
        ...
        if (aios || !deferred_done.empty()) {
          ...
          dout(20) << __func__ << " skipping flush (no aios, no deferred_done)"
                   << dendl;
        }
      }
      ...
      if (blobid_last + cct->_conf->bluestore_blobid_prealloc / 2 >
          blobid_max) {
        KeyValueDB::Transaction t =
            kv_submitting.empty() ? synct : kv_submitting.front()->t;
...
#if defined(WITH_LTTNG)
      double sync_latency =
          ceph::to_seconds<double>(mono_clock::now() - sync_start);
      for (auto txc : kv_committing) {
        if (txc->tracing) {
          tracepoint(bluestore, transaction_kv_sync_latency,
                     txc->osr->get_sequencer_id(), txc->seq,
                     kv_committing.size(), deferred_done.size(),
                     deferred_stable.size(), sync_latency);
        }
      }
#endif

      {
        ...
        if (kv_committing_to_finalize.empty()) {
          ...
          kv_committing_to_finalize.insert(kv_committing_to_finalize.end(),
                                           kv_committing.begin(),
                                           kv_committing.end());
          kv_committing.clear();
        }
        if (deferred_stable_to_finalize.empty()) {
          ...
          deferred_stable_to_finalize.insert(deferred_stable_to_finalize.end(),
...

      {
        ...
        dout(20) << __func__ << " committed " << committing_size << " cleaned "
                 << deferred_size << " in " << dur << " (" << dur_flush
                 << " flush + " << dur_kv << " kv commit)" << dendl;
        log_latency("kv_flush", l_bluestore_kv_flush_lat, dur_flush,
                    cct->_conf->bluestore_log_op_age);
        log_latency("kv_commit", l_bluestore_kv_commit_lat, dur_kv,
                    cct->_conf->bluestore_log_op_age);
        log_latency("kv_sync", l_bluestore_kv_sync_lat, dur,
                    cct->_conf->bluestore_log_op_age);
      }
        ...
        if (!bluefs_extents_reclaiming.empty()) {
          ...
          if (cct->_conf->bdev_enable_discard &&
            ...
            for (auto p = bluefs_extents_reclaiming.begin();
                 p != bluefs_extents_reclaiming.end(); ++p) {
              bdev->discard(p.get_start(), p.get_len());
            }
          }
...
void BlueStore::_kv_finalize_thread() {
  ...
  while (true) {
    ...
    if (kv_committing_to_finalize.empty() &&
      ...
      log_latency("kv_final", l_bluestore_kv_final_lat,
                  mono_clock::now() - start, cct->_conf->bluestore_log_op_age);
...
void BlueStore::_deferred_queue(TransContext *txc) {
  ...
  for (auto opi = wt.ops.begin(); opi != wt.ops.end(); ++opi) {
    ...
    for (auto e : op.extents) {
      txc->osr->deferred_pending->prepare_write(cct, wt.seq, e.offset, e.length,
                                                p);
    }
  }
  if (deferred_aggressive && !txc->osr->deferred_running) {
...
void BlueStore::_deferred_submit_unlock(OpSequencer *osr) {
  dout(10) << __func__ << " osr " << osr << " "
           << osr->deferred_pending->iomap.size() << " ios pending " << dendl;
  ...
  while (true) {
    if (i == b->iomap.end() || i->first != pos) {
      if (bl.length()) {
        dout(20) << __func__ << " write 0x" << std::hex << start << "~"
                 << bl.length() << " crc " << bl.crc32c(-1) << std::dec
                 << dendl;
    ...
    dout(20) << __func__ << "   seq " << i->second.seq << " 0x" << std::hex
             << pos << "~" << i->second.bl.length() << std::dec << dendl;
    if (!bl.length()) {
      start = pos;
    }
...
struct C_DeferredTrySubmit : public Context {
  ...
  void finish(int r) { store->deferred_try_submit(); }
};
...
int BlueStore::queue_transactions(CollectionHandle &ch,
                                  vector<Transaction> &tls, TrackedOpRef op,
  ...
  // prepare
  TransContext *txc =
      _txc_create(static_cast<Collection *>(ch.get()), osr, &on_commit);
  ...
  log_latency("submit_transact", l_bluestore_submit_lat,
              mono_clock::now() - start, cct->_conf->bluestore_log_op_age);
  log_latency("throttle_transact", l_bluestore_throttle_lat, tend - tstart,
              cct->_conf->bluestore_log_op_age);
  return 0;
}
  ...
  for (int pos = 0; i.have_op(); ++pos) {
    ...
    case Transaction::OP_SETALLOCHINT: {
      r = _set_alloc_hint(txc, c, o, op->expected_object_size,
                          op->expected_write_size, op->alloc_hint_flags);
    } break;
...
int BlueStore::_touch(TransContext *txc, CollectionRef &c, OnodeRef &o) {
...
void BlueStore::_pad_zeros(bufferlist *bl, uint64_t *offset,
  ...
  *_dout << dendl;
  ...
  dout(20) << __func__ << " pad 0x" << std::hex << front_pad << " + 0x"
           << back_pad << " on front/back, now 0x" << *offset << "~" << length
           << std::dec << dendl;
  dout(40) << "after:\n";
  bl->hexdump(*_dout);
  *_dout << dendl;
...
void BlueStore::_do_write_small(TransContext *txc, CollectionRef &c, OnodeRef o,
  ...
  do {
    ...
    if (ep != end && ep->logical_offset < offset + max_bsize) {
      ...
      dout(20) << __func__ << " considering " << *b << " bstart 0x" << std::hex
               << bstart << std::dec << dendl;
      if (bstart >= end_offs) {
        ...
        if ((b_off % chunk_size == 0 && b_len % chunk_size == 0) &&
          ...
          if (!g_conf()->bluestore_debug_omit_block_device_write) {
            if (b_len <= prefer_deferred_size) {
              ...
              b->get_blob().map_bl(
                  b_off, bl, [&](uint64_t offset, bufferlist &t) {
          ...
          Extent *le = o->extent_map.set_lextent(c, offset, b_off + head_pad,
        ...
        if (b->get_blob().get_ondisk_length() >= b_off + b_len &&
          ...
          if (!g_conf()->bluestore_debug_omit_block_device_write) {
            ...
            int r = b->get_blob().map(
                b_off, b_len, [&](uint64_t offset, uint64_t length) {
          ...
          Extent *le = o->extent_map.set_lextent(c, offset, offset - bstart,
                                                 length, b, &wctx->old_extents);
          b->dirty_blob().mark_used(le->blob_offset, le->length);
          txc->statfs_delta.stored() += le->length;
          dout(20) << __func__ << "  lex " << *le << dendl;
        ...
        if (b->can_reuse_blob(min_alloc_size, max_bsize, offset0 - bstart,
                              &alloc_len)) {
          ...
          // from garbage blobs(compressed?) can share logical space within the
          // same AU. That's in turn might be caused by unaligned len in
          // clone_range2. Hence the second write will fail in an attempt to
          // reuse blob at do_alloc_write().
          if (!wctx->has_conflict(b, offset0, offset0 + alloc_len,
                                  min_alloc_size)) {
            ...
            dout(20) << __func__ << " reuse blob " << *b << std::hex << " (0x"
                     << b_off0 << "~" << bl.length() << ")"
                     << " (0x" << b_off << "~" << length << ")" << std::dec
                     << dendl;

            o->extent_map.punch_hole(c, offset, length, &wctx->old_extents);
            wctx->write(offset, b, alloc_len, b_off0, bl, b_off, length, false,
                        false);
            logger->inc(l_bluestore_write_small_unused);
            return;
          }
    ...
    if (prev_ep != end && prev_ep->logical_offset >= min_off) {
      ...
      if (b->can_reuse_blob(min_alloc_size, max_bsize, offset0 - bstart,
        ...
        // from garbage blobs(compressed?) can share logical space within the
        // same AU. That's in turn might be caused by unaligned len in
        // clone_range2. Hence the second write will fail in an attempt to reuse
        // blob at do_alloc_write().
        if (!wctx->has_conflict(b, offset0, offset0 + alloc_len,
                                min_alloc_size)) {
  ...
  if (above_blob_threshold) {
    ...
    for (auto ep = start_ep; ep != end_ep; ++ep) {
      dout(20) << __func__ << " inserting for GC " << std::hex
               << ep->logical_offset << "~" << ep->length << std::dec << dendl;
    ...
    dout(20) << __func__ << " inserting (last) for GC " << std::hex << offset
             << "~" << length << std::dec << dendl;
  }
  // new blob.
...
void BlueStore::_do_write_big(TransContext *txc, CollectionRef &c, OnodeRef o,
  ...
  dout(10) << __func__ << " 0x" << std::hex << offset << "~" << length
           ...
           << " compress " << (int)wctx->compress << dendl;
...
int BlueStore::_do_alloc_write(TransContext *txc, CollectionRef coll,
                               OnodeRef o, WriteContext *wctx) {
  ...
  for (auto &wi : wctx->writes) {
    ...
    if (wi.compressed) {
      ...
      if (csum != Checksummer::CSUM_NONE) {
        dout(20) << __func__ << " initialize csum setting for new blob " << *b
                 ...
                 << " csum_order " << csum_order << " csum_length 0x"
    ...
    if (!g_conf()->bluestore_debug_omit_block_device_write) {
      if (l->length() <= prefer_deferred_size.load()) {
        dout(20) << __func__ << " deferring small 0x" << std::hex << l->length()
                 << std::dec << " write via deferred" << dendl;
        bluestore_deferred_op_t *op = _get_deferred_op(txc);
        op->op = bluestore_deferred_op_t::OP_WRITE;
        int r = b->get_blob().map(
            b_off, l->length(), [&](uint64_t offset, uint64_t length) {
              op->extents.emplace_back(bluestore_pextent_t(offset, length));
              return 0;
            });
        ceph_assert(r == 0);
  ...
  while (oep != wctx->old_extents.end()) {
    ...
    if (!r.empty()) {
      ...
      if (blob.is_shared()) {
        ...
        for (auto e : r) {
          b->shared_blob->put_ref(e.offset, e.length, &final, unshare_ptr);
        }
        if (unshare) {
          ceph_assert(maybe_unshared_blobs);
          maybe_unshared_blobs->insert(b->shared_blob.get());
        }
        dout(20) << __func__ << "  shared_blob release " << final << " from "
                 << *b->shared_blob << dendl;
...
void BlueStore::_choose_write_options(CollectionRef &c, OnodeRef o,
  ...
  dout(20) << __func__ << " prefer csum_order " << wctx->csum_order
           ...
           << " buffered=" << (int)wctx->buffered << std::dec << dendl;
}

int BlueStore::_do_gc(TransContext *txc, CollectionRef &c, OnodeRef o,
                      const WriteContext &wctx, uint64_t *dirty_start,
                      uint64_t *dirty_end) {
  ...
  for (auto it = extents_to_collect.begin(); it != extents_to_collect.end();
       ++it) {
    bufferlist bl;
    auto offset = (*it).first;
    auto length = (*it).second;
    dout(20) << __func__ << " processing " << std::hex << offset << "~"
             << length << std::dec << dendl;
...
int BlueStore::_do_write(TransContext *txc, CollectionRef &c, OnodeRef o,
                         uint64_t offset, uint64_t length, bufferlist &bl,
                         uint32_t fadvise_flags) {
  int r = 0;

  dout(20) << __func__ << " " << o->oid << " 0x" << std::hex << offset << "~"
           << length << " - have 0x" << o->onode.size << " (" << std::dec
           << o->onode.size << ")"
           << " bytes"
  ...
  if (wctx.extents_to_gc.empty() || wctx.extents_to_gc.range_start() > offset ||
      wctx.extents_to_gc.range_end() < offset + length) {
    benefit = gc.estimate(offset, length, o->extent_map, wctx.old_extents,
                          min_alloc_size);
  }
  ...
  if (!wctx.extents_to_gc.empty()) {
    ...
    r = _do_gc(txc, c, o, wctx, &dirty_start, &dirty_end);
    if (r < 0) {
...
int BlueStore::_write(TransContext *txc, CollectionRef &c, OnodeRef &o,
                      uint64_t offset, size_t length, bufferlist &bl,
                      uint32_t fadvise_flags) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << " 0x" << std::hex
           << offset << "~" << length << std::dec << dendl;
  int r = 0;
  ...
  dout(10) << __func__ << " " << c->cid << " " << o->oid << " 0x" << std::hex
           << offset << "~" << length << std::dec << " = " << r << dendl;
  return r;
}

int BlueStore::_zero(TransContext *txc, CollectionRef &c, OnodeRef &o,
                     uint64_t offset, size_t length) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << " 0x" << std::hex
           << offset << "~" << length << std::dec << dendl;
  int r = 0;
...
int BlueStore::_do_zero(TransContext *txc, CollectionRef &c, OnodeRef &o,
                        uint64_t offset, size_t length) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << " 0x" << std::hex
           << offset << "~" << length << std::dec << dendl;
  int r = 0;
...
void BlueStore::_do_truncate(TransContext *txc, CollectionRef &c, OnodeRef o,
                             uint64_t offset,
                             set<SharedBlob *> *maybe_unshared_blobs) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << " 0x" << std::hex
...
int BlueStore::_truncate(TransContext *txc, CollectionRef &c, OnodeRef &o,
                         uint64_t offset) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << " 0x" << std::hex
           << offset << std::dec << dendl;
  int r = 0;
...
int BlueStore::_do_remove(TransContext *txc, CollectionRef &c, OnodeRef o) {
  ...
  for (auto &s : o->extent_map.shards) {
    ...
    generate_extent_shard_key_and_apply(
        o->key, s.shard_info->offset, &key,
        [&](const string &final_key) { txc->t->rmkey(PREFIX_OBJ, final_key); });
  }
  ...
  for (auto &e : h->extent_map.extent_map) {
    const bluestore_blob_t &b = e.blob->get_blob();
    SharedBlob *sb = e.blob->shared_blob.get();
    if (b.is_shared() && std::find(unshared_blobs.begin(), unshared_blobs.end(),
...
int BlueStore::_remove(TransContext *txc, CollectionRef &c, OnodeRef &o) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << " onode " << o.get()
           << " txc " << txc << dendl;
...
int BlueStore::_setattr(TransContext *txc, CollectionRef &c, OnodeRef &o,
  ...
  if (val.is_partial()) {
    auto &b = o->onode.attrs[name.c_str()] =
        bufferptr(val.c_str(), val.length());
...
int BlueStore::_setattrs(TransContext *txc, CollectionRef &c, OnodeRef &o,
                         const map<string, bufferptr> &aset) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << " " << aset.size()
           << " keys" << dendl;
  int r = 0;
  ...
  txc->write_onode(o);
  dout(10) << __func__ << " " << c->cid << " " << o->oid << " " << aset.size()
           << " keys"
           << " = " << r << dendl;
  return r;
}

int BlueStore::_rmattr(TransContext *txc, CollectionRef &c, OnodeRef &o,
                       const string &name) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << " " << name
           << dendl;
  int r = 0;
...
int BlueStore::_rmattrs(TransContext *txc, CollectionRef &c, OnodeRef &o) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
  int r = 0;
...
void BlueStore::_do_omap_clear(TransContext *txc, OnodeRef &o) {
  ...
  dout(20) << __func__
           << " remove range start: " << pretty_binary_string(prefix)
           << " end: " << pretty_binary_string(tail) << dendl;
}

int BlueStore::_omap_clear(TransContext *txc, CollectionRef &c, OnodeRef &o) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
  int r = 0;
  if (o->onode.has_omap()) {
    o->flush();
...
int BlueStore::_omap_setkeys(TransContext *txc, CollectionRef &c, OnodeRef &o,
                             bufferlist &bl) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
  int r;
  auto p = bl.cbegin();
...
int BlueStore::_omap_setheader(TransContext *txc, CollectionRef &c, OnodeRef &o,
                               bufferlist &bl) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
  int r;
  string key;
...
int BlueStore::_omap_rmkeys(TransContext *txc, CollectionRef &c, OnodeRef &o,
                            bufferlist &bl) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
  int r = 0;
  auto p = bl.cbegin();
...
int BlueStore::_omap_rmkey_range(TransContext *txc, CollectionRef &c,
                                 OnodeRef &o, const string &first,
                                 const string &last) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
  ...
  }
  {
    ...
    dout(20) << __func__
             << " remove range start: " << pretty_binary_string(key_first)
             << " end: " << pretty_binary_string(key_last) << dendl;
  }
  txc->note_modified_object(o);
...
int BlueStore::_set_alloc_hint(TransContext *txc, CollectionRef &c, OnodeRef &o,
                               uint64_t expected_object_size,
                               uint64_t expected_write_size, uint32_t flags) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << " object_size "
           << expected_object_size << " write_size " << expected_write_size
  ...
  txc->write_onode(o);
  dout(10) << __func__ << " " << c->cid << " " << o->oid << " object_size "
           << expected_object_size << " write_size " << expected_write_size
           << " flags " << ceph_osd_alloc_hint_flag_string(flags) << " = " << r
           << dendl;
  return r;
}

int BlueStore::_clone(TransContext *txc, CollectionRef &c, OnodeRef &oldo,
                      OnodeRef &newo) {
  dout(15) << __func__ << " " << c->cid << " " << oldo->oid << " -> "
           << newo->oid << dendl;
  int r = 0;
  if (oldo->oid.hobj.get_hash() != newo->oid.hobj.get_hash()) {
    derr << __func__ << " mismatched hash on " << oldo->oid << " and "
         << newo->oid << dendl;
    return -EINVAL;
  }
...
int BlueStore::_do_clone_range(TransContext *txc, CollectionRef &c,
                               OnodeRef &oldo, OnodeRef &newo, uint64_t srcoff,
                               uint64_t length, uint64_t dstoff) {
  dout(15) << __func__ << " " << c->cid << " " << oldo->oid << " -> "
           << newo->oid << " 0x" << std::hex << srcoff << "~" << length
           << " -> "
...
int BlueStore::_clone_range(TransContext *txc, CollectionRef &c, OnodeRef &oldo,
                            OnodeRef &newo, uint64_t srcoff, uint64_t length,
                            uint64_t dstoff) {
  dout(15) << __func__ << " " << c->cid << " " << oldo->oid << " -> "
...
out:
  dout(10) << __func__ << " " << c->cid << " " << oldo->oid << " -> "
           << newo->oid << " from 0x" << std::hex << srcoff << "~" << length
           << " to offset 0x" << dstoff << std::dec << " = " << r << dendl;
  return r;
}

int BlueStore::_rename(TransContext *txc, CollectionRef &c, OnodeRef &oldo,
                       OnodeRef &newo, const ghobject_t &new_oid) {
  dout(15) << __func__ << " " << c->cid << " " << oldo->oid << " -> " << new_oid
           << dendl;
  int r;
  ...
  // rewrite shards
  {
    ...
    for (auto &s : oldo->extent_map.shards) {
      generate_extent_shard_key_and_apply(
          oldo->key, s.shard_info->offset, &key, [&](const string &final_key) {
...
out:
  dout(10) << __func__ << " " << c->cid << " " << old_oid << " -> " << new_oid
           << " = " << r << dendl;
  return r;
}
...
int BlueStore::_create_collection(TransContext *txc, const coll_t &cid,
  ...
  (*c)->flush_all_but_last();
  {
    ...
    if (r >= 0) {
      ...
      for (auto it = ls.begin(); !exists && it < ls.end(); ++it) {
        ...
        if (exists) {
          dout(1) << __func__ << " " << *it << " exists in db, "
...
void BlueStore::_do_remove_collection(TransContext *txc, CollectionRef *c) {
...
int BlueStore::_split_collection(TransContext *txc, CollectionRef &c,
...
int BlueStore::_merge_collection(TransContext *txc, CollectionRef *c,
...
void BlueStore::log_latency(const char *name, int idx, const ceph::timespan &l,
                            double lat_threshold, const char *info) const {
  logger->tinc(idx, l);
  if (lat_threshold > 0.0 && l >= make_timespan(lat_threshold)) {
...
void BlueStore::log_latency_fn(
    const char *name, int idx, const ceph::timespan &l, double lat_threshold,
    std::function<string(const ceph::timespan &lat)> fn) const {
  logger->tinc(idx, l);
  if (lat_threshold > 0.0 && l >= make_timespan(lat_threshold)) {
    dout(0) << __func__ << " slow operation observed for " << name
            << ", latency = " << l << fn(l) << dendl;
  }
}

#if defined(WITH_LTTNG)
void BlueStore::BlueStoreThrottle::emit_initial_tracepoint(
    KeyValueDB &db, TransContext &txc,
  ...
  if (should_trace(&started, &completed)) {
    txc.tracing = true;
    uint64_t rocksdb_base_level, rocksdb_estimate_pending_compaction_bytes,
        rocksdb_cur_size_all_mem_tables, rocksdb_compaction_pending,
        rocksdb_mem_table_flush_pending, rocksdb_num_running_compactions,
        rocksdb_num_running_flushes, rocksdb_actual_delayed_write_rate;
    db.get_property("rocksdb.base-level", &rocksdb_base_level);
    db.get_property("rocksdb.estimate-pending-compaction-bytes",
                    &rocksdb_estimate_pending_compaction_bytes);
    db.get_property("rocksdb.cur-size-all-mem-tables",
                    &rocksdb_cur_size_all_mem_tables);
    db.get_property("rocksdb.compaction-pending", &rocksdb_compaction_pending);
    db.get_property("rocksdb.mem-table-flush-pending",
                    &rocksdb_mem_table_flush_pending);
    db.get_property("rocksdb.num-running-compactions",
                    &rocksdb_num_running_compactions);
    db.get_property("rocksdb.num-running-flushes",
                    &rocksdb_num_running_flushes);
    db.get_property("rocksdb.actual-delayed-write-rate",
                    &rocksdb_actual_delayed_write_rate);

    tracepoint(
        bluestore, transaction_initial_state, txc.osr->get_sequencer_id(),
        txc.seq, throttle_bytes.get_current(),
        throttle_deferred_bytes.get_current(), pending_kv_ios,
        pending_deferred_ios, started, completed,
        ceph::to_seconds<double>(mono_clock::now() - start_throttle_acquire));

    tracepoint(bluestore, transaction_initial_state_rocksdb,
               txc.osr->get_sequencer_id(), txc.seq, rocksdb_base_level,
               rocksdb_estimate_pending_compaction_bytes,
               rocksdb_cur_size_all_mem_tables, rocksdb_compaction_pending,
               rocksdb_mem_table_flush_pending, rocksdb_num_running_compactions,
               rocksdb_num_running_flushes, rocksdb_actual_delayed_write_rate);
...
mono_clock::duration BlueStore::BlueStoreThrottle::log_state_latency(
  ...
  if (txc.tracing && state >= l_bluestore_state_prepare_lat &&
    ...
    tracepoint(bluestore, transaction_state_duration,
               txc.osr->get_sequencer_id(), txc.seq, state,
...
void BlueStore::BlueStoreThrottle::complete_kv(TransContext &txc) {
  ...
  if (txc.tracing) {
    tracepoint(bluestore, transaction_commit_latency,
               txc.osr->get_sequencer_id(), txc.seq,
...
void BlueStore::BlueStoreThrottle::complete(TransContext &txc) {
  ...
  if (txc.tracing) {
    ...
    tracepoint(bluestore, transaction_total_duration,
               txc.osr->get_sequencer_id(), txc.seq,
               ceph::to_seconds<double>(lat));
  }
}
...
void BlueStore::DBHistogram::update_hist_entry(
    map<string, map<int, struct key_dist>> &key_hist, const string &prefix,
  ...
  key_hist[prefix][key_slab].val_map[value_slab].max_len = std::max<size_t>(
      value_size, key_hist[prefix][key_slab].val_map[value_slab].max_len);
}
  ...
  while (iter->valid()) {
    ...
    if (key.first == PREFIX_SUPER) {
      ...
      if (key.second.back() == ONODE_KEY_SUFFIX) {
        hist.update_hist_entry(hist.key_hist, prefix_onode, key_size,
                               value_size);
        num_onodes++;
      } else {
        hist.update_hist_entry(hist.key_hist, prefix_onode_shard, key_size,
                               value_size);
        num_shards++;
      }
      ...
      hist.update_hist_entry(hist.key_hist, PREFIX_PGMETA_OMAP, key_size,
                             value_size);
      num_pgmeta_omap++;
    } else if (key.first == PREFIX_DEFERRED) {
      hist.update_hist_entry(hist.key_hist, PREFIX_DEFERRED, key_size,
                             value_size);
      num_deferred++;
      ...
      hist.update_hist_entry(hist.key_hist, PREFIX_SHARED_BLOB, key_size,
                             value_size);
      num_shared_shards++;
    } else {
...
void BlueStore::_record_onode(OnodeRef &o, KeyValueDB::Transaction &txn) {
  ...
  dout(20) << __func__ << " onode " << o->oid << " is " << bl.length() << " ("
           << onode_part << " bytes onode + " << blob_part
           << " bytes spanning blobs + " << extent_part
           << " bytes inline extents)" << dendl;
...
void BlueStore::_log_alerts(osd_alert_list_t &alerts) {
  ...
  if (!disk_size_mismatch_alert.empty()) {
    alerts.emplace("BLUESTORE_DISK_SIZE_MISMATCH", disk_size_mismatch_alert);
  }
  if (!legacy_statfs_alert.empty()) {
    alerts.emplace("BLUESTORE_LEGACY_STATFS", legacy_statfs_alert);
  }
  ...
  if (!no_per_pool_omap_alert.empty()) {
    alerts.emplace("BLUESTORE_NO_PER_POOL_OMAP", no_per_pool_omap_alert);
  }
  ...
  if (!failed_compressors.empty()) {
    ...
    alerts.emplace("BLUESTORE_NO_COMPRESSION", s0);
  }
}
...
void BlueStore::_record_allocation_stats() {
  ...
  auto t0 = std::make_tuple(alloc_stats_count.exchange(0),
                            alloc_stats_fragments.exchange(0),
                            alloc_stats_size.exchange(0));

  dout(0) << " allocation stats probe " << probe_count << ":"
          << " cnt: " << std::get<0>(t0) << " frags: " << std::get<1>(t0)
          << " size: " << std::get<2>(t0) << dendl;
  ...
  for (auto &t : alloc_stats_history) {
    dout(0) << " probe -" << base + (probe_count % base) << ": "
...
bool BlueStoreRepairer::remove_key(KeyValueDB *db, const string &prefix,
                                   const string &key) {
  if (!remove_key_txn) {
...
bool BlueStoreRepairer::fix_shared_blob(KeyValueDB *db, uint64_t sbid,
                                        const bufferlist *bl) {
  KeyValueDB::Transaction txn;
...
bool BlueStoreRepairer::fix_statfs(KeyValueDB *db, const string &key,
                                   const store_statfs_t &new_statfs) {
  if (!fix_statfs_txn) {
...
bool BlueStoreRepairer::fix_leaked(KeyValueDB *db, FreelistManager *fm,
...
bool BlueStoreRepairer::fix_false_free(KeyValueDB *db, FreelistManager *fm,
                                       uint64_t offset, uint64_t len) {
...
bool BlueStoreRepairer::fix_bluefs_extents(
    std::atomic<uint64_t> &out_of_sync_flag) {
...
uint8_t RocksDBBlueFSVolumeSelector::select_prefer_bdev(void *h) {
  ...
  case LEVEL_SLOW:
    ...
    if (db_avail4slow > 0) {
      ...
      max_db_use +=
          per_level_per_dev_max.at(BlueFS::BDEV_DB, LEVEL_WAL - LEVEL_FIRST);
      max_db_use +=
          per_level_per_dev_max.at(BlueFS::BDEV_DB, LEVEL_DB - LEVEL_FIRST);
      // this could go to db hence using it in the estimation
      max_db_use +=
          per_level_per_dev_max.at(BlueFS::BDEV_SLOW, LEVEL_DB - LEVEL_FIRST);

      auto db_total = l_totals[LEVEL_DB - LEVEL_FIRST];
      uint64_t avail =
          min(db_avail4slow, max_db_use < db_total ? db_total - max_db_use : 0);

      // considering current DB dev usage for SLOW data
      if (avail > per_level_per_dev_usage.at(BlueFS::BDEV_DB,
                                             LEVEL_SLOW - LEVEL_FIRST)) {
        res = BlueFS::BDEV_DB;
      }
    }
    break;
...
void RocksDBBlueFSVolumeSelector::get_paths(const std::string &base,
                                            paths &res) const {
...
void *
RocksDBBlueFSVolumeSelector::get_hint_by_dir(const string &dirname) const {
...
void RocksDBBlueFSVolumeSelector::dump(ostream &sout) {
  ...
  sout << "RocksDBBlueFSVolumeSelector: wal_total:"
       << l_totals[LEVEL_WAL - LEVEL_FIRST]
