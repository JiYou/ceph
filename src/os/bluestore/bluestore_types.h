...
template <typename OFFS_TYPE, typename LEN_TYPE> struct bluestore_interval_t {
...
template <> struct denc_traits<PExtentVector> {
...
struct bluestore_blob_use_tracker_t {
  ...
  bluestore_blob_use_tracker_t &
  operator=(const bluestore_blob_use_tracker_t &rhs);
...
public:
  ...
  string get_flags_string() const { return get_flags_string(flags); }
  ...
  template <class F> int map(uint64_t x_off, uint64_t x_len, F &&f) const {
  ...
  template <class F> void map_bl(uint64_t x_off, bufferlist &bl, F &&f) const {
  ...
  void init_csum(unsigned type, unsigned order, unsigned len) {
    ...
    csum_data =
        buffer::create(get_csum_value_size() * len / get_csum_chunk_size());
    csum_data.zero();
    csum_data.reassign_to_mempool(mempool::mempool_bluestore_cache_other);
  }
  ...
  bool can_prune_tail() const {
    return extents.size() > 1 && // if it's all invalid it's not pruning.
  ...
  void prune_tail() {
    ...
    if (has_csum()) {
      ...
      t.swap(csum_data);
      csum_data =
          bufferptr(t.c_str(), get_logical_length() / get_csum_chunk_size() *
                                   get_csum_value_size());
    }
  }
  void add_tail(uint32_t new_len) {
    ...
    if (has_csum()) {
      bufferptr t;
      t.swap(csum_data);
      csum_data = buffer::create(get_csum_value_size() * logical_length /
                                 get_csum_chunk_size());
...
struct bluestore_onode_t {
  ...
  void set_omap_flags() { set_flag(FLAG_OMAP | FLAG_PERPOOL_OMAP); }
  void set_omap_flags_pgmeta() { set_flag(FLAG_OMAP | FLAG_PGMETA_OMAP); }

  void clear_omap_flag() { clear_flag(FLAG_OMAP); }
...
struct bluestore_deferred_transaction_t {
  ...
  void dump(Formatter *f) const;
  static void
  generate_test_instances(list<bluestore_deferred_transaction_t *> &o);
};
...
struct bluestore_compression_header_t {
  ...
  void dump(Formatter *f) const;
  static void
  generate_test_instances(list<bluestore_compression_header_t *> &o);
};
